				...
				success : function(data) {
					...
					if(data.code == 0){
						...
						if(that.attr('data-type') == '1'){
							$('#alert_invite').html('<strong>当前商品推广规则：</strong><br/>创建推广订单后即可开始发送给好友，<b>'+parseInt(value)+'</b>人访问你的推广链接，你就可以获得<font color="red">'+res.name+'</font>！在【进度查询】页面可以看任务是否完成<br/><br/>自生成链接到任务完成限时10天，超过10天完成的则属于无效，不发放奖励，奖励到账时间24小时内');
						}else{
