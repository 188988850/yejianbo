		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.code >= 0){
					$('#alert_frame').hide();
					alert('领取成功！');
					if(data.is_video_order == 1){
						alert('【调试】检测到 is_video_order=1，页面将刷新');
						console.log('【调试】is_video_order=1，data=', data);
						window.location.reload();
					}else{
						alert('【调试】不是视频订单，跳转订单页');
						console.log('【调试】不是视频订单，data=', data);
...
(function(){
    function logJump(type, url){
        try {
            console.log('[跳转监视]', type, url, new Error().stack);
        }catch(e){}
        alert('[跳转监视] ' + type + ': ' + url + '\n' + (new Error().stack));
    }
