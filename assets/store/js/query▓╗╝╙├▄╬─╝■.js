	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if(data.complain){
					item += '<tr><td class="orderTitle">订单操作</td><td class="orderContent"><a href="./user/workorder.php?my=add&orderid='+id+'&skey='+skey+'" target="_blank" onclick="return checklogin('+data.islogin+')" class="text-primary">售后反馈</a>';
				...
				item += '<tbody class="tbody"><tr><td colspan="6" style="text-align:center" class="orderTitle"></td></tr>';
				item += '<tr><td colspan="6" class="orderTitle page-title" style="padding:0"><b>订单实时状态</b></td></tr></tbody>' +
					'<tbody class="tbody" style="background: #f7f7f8;border-radius: 10px " border="10px" >'+
					...
					item += '<tr><td colspan="6"  class="orderTitle page-title" style="padding:5px 15px"><b>订单实时状态</b></tr>';
						$.each(data.list, function(i, v){
							item += '<tr><td class="orderTitle" style="color: #ff0000;">'+i+'：'+v+'</td></tr>';
						});
					}
				item += '</tbody><tbody>';
				}else if(data.kminfo){
