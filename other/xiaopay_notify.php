...
function processOrder2($srow, $is_fenzhan = true)
{
	...
	if ($srow['tid'] == -2) {
		...
		if ($type == "update") {
			...
			$sql="INSERT INTO `shua_site` (`upzid`,`power`,`domain`,`user`,`pwd`,`rmb`,`qq`,`sitename`,`description`,`anounce`,`alert`,`kfqq`,`addtime`,`endtime`,`status`) VALUES (:upzid, :power, :domain, :user, :pwd, :rmb, :qq, :sitename, :description, NULL, NULL, :kfqq, NOW(), :endtime, 1)";
		...
		for ($i = 0; $i < $cart_num; $i++) {
			...
			if ($cart_item && $cart_item['input']) {
				if ($cart_id > 0 && $cart_num > 0) {
					...
					$DB->exec("INSERT INTO `shua_orders` (`tid`,`zid`,`input`,`input2`,`input3`,`input4`,`input5`,`value`,`userid`,`addtime`,`tradeno`,`money`,`cost`,`status`,`djzt`,`vid`) VALUES ('" . $cart_item['tid'] . "','" . $srow['zid'] . "','" . addslashes($input[0]) . "','" . addslashes($input[1]) . "','" . addslashes($input[2]) . "','" . addslashes($input[3]) . "','" . addslashes($input[4]) . "','" . $cart_item['num'] . "','" . $srow['userid'] . "','" . $date . "','cart" . $srow['trade_no'] . "','" . $cart_item['money'] . "','" . $cost . "','0','" . ($tools['is_curl'] == 2 ? 2 : 0) . "','" . $cart_item['vid'] . "')");
					...
					if (do_goods($orderid)) {
						...
						if ($srow['inviteid'] > 0 && $conf['invite_tid'] && ($inviteshop_row['value'] > 0 && $srow['money'] >= $inviteshop_row['value'])) {
							if ($invitelog_row && $invitelog_row['status'] == 0 && $invite_row && $invite_row['status'] == 0 && $inviteshop_row && $invite_row['active'] == 1) {
								...
								$DB->exec("INSERT INTO `shua_orders` (`tid`,`zid`,`input`,`value`,`status`,`djzt`,`money`,`cost`,`tradeno`,`addtime`,`endtime`,`vid`) VALUES (" . $inviteshop_row['tid'] . ",'" . $srow['zid'] . "','" . $qq . "',1,0,2,'0','0','invite" . $srow['trade_no'] . "','" . $date . "','" . $date . "','" . $cart_item['vid'] . "')");
	...
	$DB->exec("INSERT INTO `shua_orders` (`tid`,`zid`,`input`,`input2`,`input3`,`input4`,`input5`,`value`,`userid`,`addtime`,`tradeno`,`money`,`cost`,`status`,`djzt`,`vid`) VALUES ('" . $srow['tid'] . "','" . $srow['zid'] . "','" . addslashes($input[0]) . "','" . addslashes($input[1]) . "','" . addslashes($input[2]) . "','" . addslashes($input[3]) . "','" . addslashes($input[4]) . "','" . $srow['num'] . "','" . $srow['userid'] . "','" . $date . "','" . $srow['trade_no'] . "','" . $srow['money'] . "','" . $cost . "','" . $status . "','" . ($tools['is_curl'] == 2 ? 2 : 0) . "','" . $srow['vid'] . "')");
	$orderid = $DB->lastInsertId();
	if (!$orderid) {
		return false;
	}
			...
			if ($shequ && $shequ['type'] && $shequ['username'] && $shequ['password']) {
				...
				if ($result['faka'] == true) {
					...
					foreach ($result['kmdata'] as $km_arr) {
						$DB->query("INSERT INTO `shua_faka` (`tid`,`km`,`pw`,`orderid`,`addtime`,`usetime`,`vid`) VALUES ('" . $srow['tid'] . "','" . $km_arr['card'] . "','" . $km_arr['pass'] . "','" . $orderid . "',NOW(),NOW(),'" . $cart_item['vid'] . "')");
