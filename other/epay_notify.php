    ...
    if ($_GET['trade_status'] == 'TRADE_SUCCESS' && $srow['status']==0 && round($srow['money'],2)==round($money,2)) {
		...
		if($DB->exec("UPDATE `pre_pay` SET `status` ='1' WHERE `trade_no`='{$out_trade_no}'")){
			...
			if($payorder['name']=='自助开通站点' || $payorder['name']=='自助开通站点'){
			     processOrder2($srow);   
			}elseif(strpos($payorder['name'],'购买影视') !== false){
			     processOrder2($srow);   
			}else{
		         processOrder($srow);
			} 
...
function processOrder2($srow, $is_fenzhan = true)
{
	...
	if ($srow['tid'] == -4) {
	    ...
	    $goods_type = isset($inputArr[1]) ? $inputArr[1] : '';
	    ...
	    $DB->exec("INSERT INTO `shua_orders` (`vid`,`zid`,`input`,`input2`,`input3`,`value`,`userid`,`addtime`,`tradeno`,`money`,`cost`,`status`,`djzt`) VALUES ('" . $srow['vid'] . "','" . $srow['zid'] . "','" . $input . "','" . $input2 . "','" . $input3 . "','" . $srow['num'] . "','" . $srow['userid'] . "','" . $date . "','" . $srow['trade_no'] . "','" . $srow['money'] . "','" . $cost . "','" . $status . "','" . ($tools['is_curl'] == 2 ? 2 : 0) . "')");
