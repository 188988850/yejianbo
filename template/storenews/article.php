...
$row=$DB->getRow("select * from shua_article where id='$id' and active=1 limit 1");
if(!$row)
	sysmsg('当前文章不存在！');
$downResult = $DB->getRow("SELECT * FROM shua_article WHERE id<'$id' AND active=1 ORDER BY id DESC LIMIT 1");
$upResult = $DB->getRow("SELECT * FROM shua_article WHERE id>'$id' AND active=1 ORDER BY id DESC LIMIT 1");
$DB->exec("UPDATE `shua_article` SET `count`=`count`+1 WHERE id='$id'");
?>
<!DOCTYPE html>
<html lang="zh-cn">
<head>
