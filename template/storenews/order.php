...
if (empty($_SESSION['zid'])) {
    header('Location: /template/storenews/user/login.php?return_url=' . urlencode($_SERVER['REQUEST_URI']));
    exit;
}

$orderid=trim(daddslashes($_GET['orderid']));
$row=$DB->getRow("select * from shua_pay where trade_no='$orderid' limit 1");
if(!$row)sysmsg('当前订单不存在');
if($row['status']==1)exit("<script language='javascript'>alert('当前订单已完成支付！');window.location.href='./?buyok=1';</script>");

$tool=$DB->getRow("SELECT A.*,B.blockpay FROM shua_tools A LEFT JOIN shua_class B ON A.cid=B.cid WHERE tid='{$row['tid']}' LIMIT 1");
if($tool['is_curl']==4){
	$isfaka=true;
}
...
<body style="background:#f5f5f5;overflow-x: hidden; ">
    <div class="layui-row layui-col-space8">
        <div class="layui-col-xs12" style="margin-top: 0.5em;padding: 0.5em;">
        </div>
                ...
                <div class="layui-row layui-col-space8">
                    <div class="layui-col-xs2">
                        <div style="border-radius: 0.5em;text-align: center;margin: 0.8em;box-shadow: 3px 3px 16px #eee" id="layer-photos-demo" class="layer-photos-demo">
                            <img alt="<?php echo isset($row['name']) && $row['name'] ? $row['name'] : $tool['name']; ?>" layer-src="<?php echo isset($row['shopimg']) && $row['shopimg'] ? $row['shopimg'] : (isset($tool['shopimg']) && $tool['shopimg'] ? $tool['shopimg'] : 'assets/store/picture/error_img.png'); ?>" src="<?php echo isset($row['shopimg']) && $row['shopimg'] ? $row['shopimg'] : (isset($tool['shopimg']) && $tool['shopimg'] ? $tool['shopimg'] : 'assets/store/picture/error_img.png'); ?>" style="width: 100%;border-radius: 0.5em"/>
                        </div>
                    </div>
                    <div class="layui-col-xs10">
                        <div class="layui-card-header" style="font-size: 1.1em;line-height: 2em;height:auto;">
                            <?php echo isset($row['name']) && $row['name'] ? $row['name'] : $tool['name']; ?>
                        </div>
                        <div class="layui-card-body information layui-text" style="line-height: 2em">
                            <?php if(isset($row['tid']) && $row['tid'] == -4): ?>
                                <p>类型：<?php echo $row['buytype']; ?></p>
                                <?php if($row['episodes']) { ?>
                                    <p>集数：<?php echo $row['episodes']; ?></p>
                                <?php } ?>
                                <p>简介：<?php echo $row['desc']; ?></p>
                                <?php if($row['download_url']) { ?>
                                    <p>下载地址：<a href="<?php echo $row['download_url']; ?>" target="_blank">点击下载</a></p>
                                <?php } ?>
                            <?php endif; ?>
                            <p>购买数量<span style="float: right;"><?php echo $row['num']; ?>个</span></p>
                            <p>商品售价<span style="float: right;"><?php echo $row['money']; ?> 元</span></p>
                            <?php if(!isset($row['tid']) || $row['tid'] != -4): ?>
                                <p>售价等级<span style="float: right;"><?php echo $level; ?></span></p>
                                <p>下单信息<span style="float: right;"><?php echo $show; ?></span></p>
                                <p style="float: right;margin-top: 0.8em;font-size: 1.05em">共<?php echo $row['num']; ?>份 小计：<font color="#ff4500" style="font-size: 1.1em">￥<?php echo $row['money']; ?></font></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="layui-col-xs12" style="padding: 0.1em 1em 0 1em;margin-top: 0.9em;margin-bottom: 1em;">
            <div class="layui-card" style="border-radius: 0.5em">
       ...
       <div class="layui-col-sm12" style="margin-top: 0.5em;">
            ...
            <div class="list-group-item reed" align="center">
                <a style="font-size: 14px;color:#ff0000;"><b>【推荐使用余额下单更方便快捷】</b></a><br><br>
                <?php echo $conf['czsm2']; ?>
            </div>
                <div class="layui-card-body layui-form layui-form-pane" style="padding: 0.5em">
                    ...
                    <div style="padding: 0.3em; border-radius: 0.3em; line-height: 2em; height: 2.5em;">
                        <input type="radio" name="pay" value="rmb" title="<p style='width:100%'><img src='./assets/img/rmb.png' width='13px' /> 余额 <span style='color:#999;font-size:0.8em;margin-left:1em'>剩<?php echo $userrow['rmb']; ?>元</span></p>" checked>
                    <span>
                    <a style='color:#ff0000;font-size:1em;margin-left:1em' target="_blank" href="/user/recharge.php">
                         <b>【充值余额点我】</b>
                    </a>
                </span>
            </div>
    ...
    <span style="color: #FF0000;"><strong>&nbsp;&nbsp;截图领取支付宝红包码[领券下单更优惠]</strong></span>
        ...
        <div class="layui-col-xs12" style="max-width: 550px;box-shadow: 0px 0px 2px #ccc;text-align: right;z-index: 3;bottom: 0;left: 0;height: 4em;line-height: 4em;background-color: white;">
            ...
            <span style="font-size: 1em;float: left;text-indent: 0.5em"><span>￥</span><span style="color: #ff4500;font-size: 1.3em;"><?php echo $row['money'] ?></span><span style="color: #ff4500;font-size: 0.8em">元</span>&nbsp;<span style="color: #A6A6A6;font-size: 0.8em">x<?php echo $row['num'] ?>份</span></span>
            <button class="layui-btn layui-btn-danger layui-btn-radius" style="background: linear-gradient(to right, #f85032, #e73827);margin-right: 0.5em;" id="dopay">提交支付
            </button>
        </div>
