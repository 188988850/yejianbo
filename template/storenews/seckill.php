...
$rs=$DB->query("SELECT A.*,B.price as oldprice,B.name,B.shopimg FROM shua_seckillshop A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE A.active=1 ORDER BY A.sort ASC");
$seckill = array();
while($res = $rs->fetch())
{
	$seckill[] = $res;
