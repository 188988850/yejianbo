...
if(isset($_GET['kw'])){
	...
	$sql=" B.name LIKE '%$kw%'";
	$numrows=$DB->getColumn("SELECT count(*) FROM pre_seckillshop A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE{$sql}");
...
$rs=$DB->query("SELECT A.*,B.name FROM pre_seckillshop A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE{$sql} ORDER BY A.sort ASC LIMIT $offset,$pagesize");
while($res = $rs->fetch())
{
