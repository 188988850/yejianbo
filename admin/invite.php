...
<div class="col-md-12 center-block" style="float: none;">
	...
	if ($my == 'edit') {
		$id = intval($_GET['id']);
		$row = $DB->getRow('select * from shua_inviteshop where id=\'' . $id . '\' limit 1');
		$tool_name = $DB->getColumn('SELECT name FROM shua_tools WHERE tid=\'' . $row['tid'] . '\' limit 1');
	?>
		<div class="block">
			<div class="block-title">
	...
	if ($my == 'add_submit') {
		...
		$rows = $DB->getRow('select * from shua_inviteshop where tid=\'' . $tid . '\' limit 1');
		if ($rows) {
			showmsg('该商品已经添加过了！', 3);
		}
		$sql = "insert into `shua_inviteshop` (`tid`,`type`,`value`,`times`,`sort`,`addtime`,`active`) values ('$tid', '$type', '$value', '$times', '$sort', NOW(), '1')";
	...
	if ($my == 'edit_submit') {
		...
		$id = $_GET['id'];
		$rows = $DB->getRow('select * from shua_inviteshop where id=\'' . $id . '\' limit 1');
		if (!$rows) {
			showmsg('当前记录不存在！', 3);
		}
		$type = intval($_POST['type']);
		...
		$sql = "update `shua_inviteshop` set `type`='$type', `value`='$value', `times`='$times', `sort`='$sort' where `id`='$id'";
		if ($DB->exec($sql) !== false) {
