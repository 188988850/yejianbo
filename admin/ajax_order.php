...
case 'setStatus':
	...
	if($status==5){
		if($DB->exec("DELETE FROM shua_orders WHERE id='$id'")!==false)
			exit('{"code":200}');
		else
			exit('{"code":400,"msg":"删除订单失败！'.$DB->error().'"}');
	}else{
		if($DB->exec("update shua_orders set status='$status',result=NULL where id='{$id}'")!==false)
			exit('{"code":200}');
		else
...
case 'getmoney': //退款查询
	...
	//$tc_point=$DB->getColumn("select point from shua_points where zid='{$row['zid']}' and action='提成' and orderid='$id' limit 1");
