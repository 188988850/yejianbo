...
if($_GET['cid']){
	$cid = intval($_GET['cid']);
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools where is_curl=4 and cid='$cid'");
	$sql=" is_curl=4 and cid='$cid'";
	$link="&cid=".$cid;
}elseif($_GET['tid']){
	$tid = intval($_GET['tid']);
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools where is_curl=4 and tid='$tid'");
	$sql=" is_curl=4 and tid='$tid'";
	$link="&tid=".$tid;
}else{
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools where is_curl=4");
	$sql=" is_curl=4";
}
