...
case 'getcount':
	...
	if($isUpdate){
		...
		$count77=$DB->getColumn("SELECT count(*) FROM pre_orders WHERE status=2");
		$count78=$DB->getColumn("SELECT count(*) FROM pre_orders WHERE status=3");
...
case 'delvideo':
	$id=intval($_GET['id']);
	$sql="DELETE FROM pre_video WHERE id='$id' limit 1";
	if($DB->exec($sql)!==false){
		exit('{"code":0,"msg":"删除成功！"}');
	}else{
		exit('{"code":-1,"msg":"删除失败！'.$DB->error().'"}');
...
case 'delvideolist':
	$id=intval($_GET['id']);
	$sql="DELETE FROM pre_videolist WHERE id='$id' limit 1";
	if($DB->exec($sql)!==false){
		exit('{"code":0,"msg":"删除成功！"}');
	}else{
		exit('{"code":-1,"msg":"删除失败！'.$DB->error().'"}');
...
case 'delvideotype':
	$id=intval($_GET['id']);
	$sql="DELETE FROM pre_videotype WHERE id='$id' limit 1";
	if($DB->exec($sql)!==false){
		exit('{"code":0,"msg":"删除成功！"}');
	}else{
		exit('{"code":-1,"msg":"删除失败！'.$DB->error().'"}');
...
case 'get_videotype':
    ...
    if(!$row)exit('{"code":-1,"msg":"当前记录不存在！"}');
    exit(json_encode(['code'=>0,'msg'=>'succ','data'=>$row]));
    break;
default:
