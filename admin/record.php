...
$income_today=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='提成' AND{$sql} AND addtime>'$thtime'");
$outcome_today=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='消费' AND{$sql} AND addtime>'$thtime'");
$income_lastday=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='提成' AND{$sql} AND addtime<'$thtime' AND addtime>'$lastday'");
$outcome_lastday=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='消费' AND{$sql} AND addtime<'$thtime' AND addtime>'$lastday'");
if(isset($_GET['zid'])){
$income_all=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='提成' AND{$sql}");
$outcome_all=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE action='消费' AND{$sql}");
}
...
<?php
$numrows=$DB->getColumn("SELECT count(*) from shua_points WHERE{$sql}");
...
while($res = $rs->fetch())
{
echo '<tr><td><b>'.$res['id'].'</b></td><td><a href="sitelist.php?zid='.$res['zid'].'" target="_blank">'.$res['zid'].'</a></td><td>'.$res['action'].'</td><td>'.($res['orderid']?'<a href="./list.php?id='.$res['orderid'].'" target="_blank">'.$res['orderid'].'</a>':'无').'</td><td><font color="'.(in_array($res['action'],array('提成','奖励','赠送','退款','退回','充值','加款'))?'red':'green').'">'.$res['point'].'</font></td><td>'.$res['bz'].'</td><td>'.$res['addtime'].'</td></tr>';
}
?>
          </tbody>
        </table>
