...
<div class="col-xs-12 col-md-10 center-block" style="float: none;">
    ...
    $cron_lasttime = $DB->getColumn("SELECT `v` FROM `shua_config` WHERE `k` = 'cron_rank_time' LIMIT 1");
    $cron_money    = $DB->getColumn("SELECT `v` FROM `shua_config` WHERE `k` = 'cron_rank_money' LIMIT 1");
    ?>
    <div class="block">
        ...
        <?php
            ...
            $limit   = $conf['rank_reward'] > 10 ? $conf['rank_reward'] : 999;
            if ($_GET['last'] == 1) {
