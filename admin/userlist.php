...
<div class="col-md-12 center-block" style="float: none;">
	...
	if ($my == 'add_submit') {
		...
		$rows = $DB->getRow('select * from shua_site where user=\'' . $user . '\' limit 1');
		if ($rows) {
			showmsg('用户名已存在！', 3);
		}
	...
	if ($my == 'edit_submit') {
		...
		$sql = "update shua_site set rmb = '$rmb',qq = '$qq' $sql,qq_openid = '$qq_openid',wx_openid = '$wx_openid' where zid= '$zid'";
		if ($DB->exec($sql) !== false) {
