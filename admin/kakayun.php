...
if($act == 'ajax_goods'){
    ...
    foreach ($goods['data'] as $key => $item){
        $tool=$DB->getRow("SELECT * FROM shua_tools WHERE shequ=:shequ AND goods_id=:goods_id LIMIT 1", [':shequ'=>$shequ, ':goods_id'=>$item['goodsid']]);
        if($tool){
...
if($act == 'ajax_post'){
    ...
    // 获取商品数据
    $qu = $DB->getRow('select * from shua_shequ where id=\'' . $shequ . '\' limit 1');
    if ($qu['type'] != 'kakayun') {
...
<?php if(!$act){ ?>
<?php
    $shequlist = $DB->getAll('SELECT * FROM shua_shequ WHERE type=\'kakayun\' order by id asc');
    $shequselect = '';
...
$classlist = $DB->getAll('SELECT * FROM shua_class WHERE 1 order by sort asc');
