...
$DB->exec("TRUNCATE TABLE `shua_logs`");
showmsg('清空社区对接日志成功！',1);
}elseif($mod=='cleanpay'){
$DB->exec("DELETE FROM `shua_pay` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-90 days"))."'");
$DB->exec("DELETE FROM `shua_pay` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-3 hours"))."' and status=0");
$DB->exec("DELETE FROM `shua_cart` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-30 days"))."'");
$DB->exec("DELETE FROM `shua_cart` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-12 hours"))."' and status<2");
$DB->exec("OPTIMIZE TABLE `shua_pay`");
showmsg('删除90天前支付记录成功！',1);
}elseif($mod=='cleanorders'){
$DB->exec("DELETE FROM `shua_orders` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-90 days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_orders`");
showmsg('删除90天前订单记录成功！',1);
}elseif($mod=='cleanqiandao'){
$DB->exec("DELETE FROM `shua_qiandao` WHERE time<'".date("Y-m-d H:i:s",strtotime("-30 days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_qiandao`");
showmsg('删除30天前签到记录成功！',1);
}elseif($mod=='cleanwork'){
$DB->exec("DELETE FROM `pre_workorder` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-90 days"))."'");
$DB->exec("OPTIMIZE TABLE `pre_workorder`");
showmsg('删除90天前工单记录成功！',1);
}elseif($mod=='cleanpoints'){
$DB->exec("DELETE FROM `shua_points` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-90 days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_points`");
showmsg('删除90天前收支明细成功！',1);
}elseif($mod=='cleangift'){
$DB->exec("DELETE FROM `shua_giftlog` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-1 days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_giftlog`");
showmsg('删除1天前中奖记录成功！',1);
}elseif($mod=='cleaninvite'){
$DB->exec("DELETE FROM `shua_invitelog` WHERE date<'".date("Y-m-d H:i:s",strtotime("-10 days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_invitelog`");
showmsg('删除10天前推广记录成功！',1);
}elseif($mod=='cleanpayi' && $_POST['do']=='submit'){
$days = intval($_POST['days']);
$money = daddslashes($_POST['money']);
if($days<=0 || $money==null)showmsg('请确保每项不能为空',3);
$DB->exec("DELETE FROM `shua_pay` WHERE money<='$money' and addtime<'".date("Y-m-d H:i:s",strtotime("-{$days} days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_pay`");
showmsg('删除支付记录成功！',1);
...
$money = daddslashes($_POST['money']);
if($days<=0 || $money==null)showmsg('请确保每项不能为空',3);
$DB->exec("DELETE FROM `shua_orders` WHERE money<='$money' and addtime<'".date("Y-m-d H:i:s",strtotime("-{$days} days"))."'");
$DB->exec("OPTIMIZE TABLE `shua_orders`");
showmsg('删除订单记录成功！',1);
...
$money = daddslashes($_POST['money']);
if($days<=0 || $money==null)showmsg('请确保每项不能为空',3);
$DB->exec("DELETE FROM `shua_site` WHERE rmb<='$money' and addtime<'".date("Y-m-d H:i:s",strtotime("-{$days} days"))."' and (lasttime<'".date("Y-m-d H:i:s",strtotime("-{$days} days"))."' or lasttime is null)");
$DB->exec("OPTIMIZE TABLE `shua_pay`");
showmsg('删除分站记录成功！',1);
}else{
?>
...
<a href="./clean.php?mod=cleanpay" onclick="return confirm('你确实要删除90天前的支付记录吗？');" class="btn btn-block btn-default">删除90天前支付记录</a><br/>
<a href="./clean.php?mod=cleanorders" onclick="return confirm('你确实要删除90天前的订单记录吗？');" class="btn btn-block btn-default">删除90天前订单记录</a><br/>
<a href="./clean.php?mod=cleanpoints" onclick="return confirm('你确实要删除90天前收支明细吗？');" class="btn btn-block btn-default">删除90天前收支明细</a><br/>
<a href="./clean.php?mod=cleanwork" onclick="return confirm('你确实要删除90天前的工单记录吗？');" class="btn btn-block btn-default">删除90天前工单记录</a><br/>
<a href="./clean.php?mod=cleangift" onclick="return confirm('你确实要删除1天前的中奖记录吗？');" class="btn btn-block btn-default">删除1天前中奖记录</a><br/>
<a href="./clean.php?mod=cleaninvite" onclick="return confirm('你确实要删除30天前的推广记录吗？');" class="btn btn-block btn-default">删除10天前推广记录</a><br/>
