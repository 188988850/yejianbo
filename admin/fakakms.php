...
if(isset($_GET['tid'])){
	$tid=intval($_GET['tid']);
	$row=$DB->getRow("select cid,name from shua_tools where tid='$tid' limit 1");
...
<div class="form-group">
  <div class="input-group">
    <span class="input-group-addon">批量数量</span>
    <input type="number" min="2" id="batchAddCount" class="form-control" value="2" style="max-width:100px;">
    <span class="input-group-btn">
      <button type="button" class="btn btn-info" id="batchAddBtn">批量添加相同卡密</button>
      <button type="button" class="btn btn-warning" id="undoBtn" title="撤销最后一次批量添加">撤销</button>
    </span>
  </div>
  <div class="input-group" style="margin-top:5px;">
    <span class="input-group-addon">固定数量</span>
    <input type="number" min="2" id="fixedCount" class="form-control" value="2" style="max-width:100px;">
    <span class="input-group-btn">
      <button type="button" class="btn btn-success" id="saveFixedCount">保存设置</button>
    </span>
  </div>
  <div style="color:#888;font-size:12px;margin-top:2px;">将当前卡密内容复制为多行，数量为上方输入值</div>
</div>
<!-- 批量添加相同卡密功能 结束 -->
<div class="form-group">
	<div class="input-group">
...
if($tid==NULL or $kms==NULL){
	...
	{	
		...
		if($is_check_repeat==1){
			if($DB->getRow("select * from shua_faka where km='$km' limit 1"))continue;
		}
		$sql=$DB->exec("INSERT INTO `shua_faka` (`tid`,`km`,`pw`,`addtime`) VALUES ('".$tid."','".$km."','".$pw."',NOW())");
		if($sql)$c++;
...
if(!checkRefererHost())exit;
$id=intval($_GET['id']);
$sql=$DB->exec("DELETE FROM shua_faka WHERE kid='$id'");
...
<div class="block-title"><h3 class="panel-title">清空卡密</h3></div>
<div class="box">';
if($DB->exec("DELETE FROM shua_faka WHERE tid='$tid'")!==false){
...
<div class="block-title"><h3 class="panel-title">清空卡密</h3></div>
<div class="box">';
if($DB->exec("DELETE FROM shua_faka WHERE tid='$tid' and orderid!=0")!==false){
echo '<div class="box">清空成功.</div>';
}else{
echo'<div class="box">清空失败.</div>';
}
...
foreach($checkbox as $kid){
	$DB->exec("DELETE FROM shua_faka WHERE kid='$kid' limit 1");
	$i++;
}
...
$numrows=$DB->getColumn("SELECT count(*) from shua_faka WHERE{$sql}");
?>
...
$(function(){
  // 从本地存储加载固定数量
  var fixedCount = localStorage.getItem('faka_fixed_count');
  if(fixedCount) {
    $('#fixedCount').val(fixedCount);
    $('#batchAddCount').val(fixedCount);
  }
  
  // 保存固定数量
  $('#saveFixedCount').click(function(){
    var count = parseInt($('#fixedCount').val());
    if(isNaN(count)||count<2){
      layer.alert('请输入大于1的数量');
      return;
    }
    localStorage.setItem('faka_fixed_count', count);
    $('#batchAddCount').val(count);
    layer.msg('设置已保存');
  });
  ...
  $('#batchAddBtn').click(function(){
    var count = parseInt($('#batchAddCount').val());
    var content = $('#kms').val();
    if(!content){layer.alert('请先输入卡密内容');return;}
    if(isNaN(count)||count<2){layer.alert('请输入大于1的数量');return;}
    // 保存当前内容用于撤销
    lastContent = content;
    var lines = [];
    for(var i=0;i<count;i++) lines.push(content);
    $('#kms').val(lines.join('\n'));
  });
