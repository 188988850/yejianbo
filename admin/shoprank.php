...
if($_GET['last']==0){
	$sql = "SELECT B.tid,B.name,count(A.id) num FROM shua_orders A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE A.addtime>='$thtime' GROUP BY B.tid ORDER BY num DESC LIMIT 999";
}
if($_GET['last']==1){
	$sql = "SELECT B.tid,B.name,count(A.id) num FROM shua_orders A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE A.addtime>='$lastday' AND A.addtime<'$thtime' GROUP BY B.tid ORDER BY num DESC LIMIT 999";
}
if($_GET['last']==2){
