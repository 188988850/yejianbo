...
case 'delSite': //删除站点
	...
	$zid=intval($_GET['zid']);
	if($DB->exec("DELETE FROM shua_site WHERE zid='$zid'")!==false)
...
case 'setSite': //开启关闭站点
	...
	$DB->exec("update shua_site set status='$active' where zid='{$zid}'");
	exit('{"code":0,"msg":"succ"}');
break;
case 'setSuper': //切换站点版本
	adminpermission('site', 2);
	$zid=intval($_GET['zid']);
	$row=$DB->getRow("select * from shua_site where zid='$zid' limit 1");
	if($row['power']==2)$power=1;
	else $power=2;
	$DB->exec("update shua_site set power='$power' where zid='{$zid}'");
	exit('{"code":0,"msg":"succ"}');
break;
case 'setEndtime': //分站延时
	...
	$DB->exec("update shua_site set endtime='$endtime' where zid='{$zid}'");
	exit('{"code":0,"msg":"succ"}');
break;
case 'siteRecharge': //分站充值
	...
	$row=$DB->getRow("select zid,rmb from shua_site where zid='$zid' limit 1");
	if(!$row)
...
case 'opTixian': //操作提现
	...
	if($op == 'delete'){
		...
		$rows=$DB->getRow("select * from pre_tixian where id='$id' limit 1");
		$reason=$conf['fenzhan_skimg']==1?'有用户订单退款，资金驳回！':'有用户订单退款，资金驳回！';
...
case 'cleariprice':
	adminpermission('site', 2);
	$zid=intval($_POST['zid']);
	if($DB->exec("update shua_site set iprice=NULL where zid='{$zid}'")!==false)
		exit('{"code":0,"msg":"succ"}');
	else
