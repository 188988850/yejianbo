...
$rs=$DB->query("SELECT * FROM shua_faka WHERE {$sql} order by kid asc{$limit}");
$data='';
while($res = $rs->fetch())
{
	$data.=($res['pw']?$res['km'].' '.$res['pw']:$res['km'])."\r\n";
	if($_GET['isuse']==1&&$_GET['use']==0)$DB->exec("update `shua_faka` set orderid=1,usetime=NOW() where `kid`='{$res['kid']}'");
}
...
if($tid>0){
	...
	$rs=$DB->query("SELECT tid,value FROM shua_tools WHERE cid='$cid'");
	$tids='';
	while($res = $rs->fetch())
	{
...
$rs=$DB->query("SELECT * FROM shua_orders WHERE {$sql} and status={$status} order by id {$orderby} limit 1000");
...
while($row = $rs->fetch())
{
	...
	if($sign>0)$DB->exec("update `shua_orders` set status={$sign} where `id`='{$row['id']}'");
}
}
