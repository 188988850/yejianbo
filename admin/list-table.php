...
while($res = $rs->fetch())
{
    ...
    if($res['tid']==-4){
        ...
        $type = isset($inputArr[1]) ? $inputArr[1] : '';
        $episodes = isset($inputArr[2]) ? $inputArr[2] : '';
        $videolist = $DB->getRow("SELECT name FROM shua_videolist WHERE id='{$vid}' LIMIT 1");
        $name = $videolist ? $videolist['name'] : '未知短剧';
        // 类型标记
        if($type == 'url'){
            $name .= ' [网盘]';
        }elseif($type == 'play' && $episodes == 'all'){
            $name .= ' [全集]';
        }elseif($type == 'play' && is_numeric($episodes)){
            $name .= ' [第'.$episodes.'集]';
        }
    }else{
        $name = $res['name'];
    ...
    // 友好展示 input 字段
    $input_show = $res['input'];
    if(isset($res['display_input']) && $res['display_input']){
        $input_show = $res['display_input'];
    }
    $input_detail = $input_show;
    if($res['input2']) $input_detail .= '<br/>'.$res['input2'];
    if($res['input3']) $input_detail .= '<br/>'.$res['input3'];
    if($res['input4']) $input_detail .= '<br/>'.$res['input4'];
    if($res['input5']) $input_detail .= '<br/>'.$res['input5'];
    
    echo '<tr><td><input type="checkbox" name="checkbox[]" id="list1" value="'.$res['id'].'" onClick="unselectall1()"><b>'.$res['id'].'</b></td><td><span onclick="showOrder('.$res['id'].')" title="点击查看详情">'.$name.'</span></td><td class="wbreak"><span onclick="inputOrder('.$res['id'].')" title="点击修改数据">'.$input_detail.'</span></td><td><span onclick="inputNum('.$res['id'].')" title="点击修改份数">'.$res['value'].'</span></td><td><a href ="sitelist.php?zid='.$res['zid'].'" target="_blank">'.$res['zid'].'</a></td><td>'.(strlen($res['userid'])!=32?'<a href ="userlist.php?zid='.$res['userid'].'" target="_blank">'.$res['userid'].'</a>':'0').'</td><td>'.$res['money'].'</td><td>'.$res['addtime'].'</td><td>'.display_djzt($res['djzt'],$res['id']).'</td><td>'.display_zt($res['status'],$res['id']).'</td><td><a href="./shoplist.php?&kw='.$res['name'].'">查看商品</a></td><td><select onChange="javascript:setStatus(\''.$res['id'].'\',this.value)" class="form-control"><option selected>操作订单</option><option value="0">待处理</option><option value="2">正在处理</option><option value="1">已完成</option><option value="4">已退单</option><option value="3">异常</option>'.($res['zid']>1||is_numeric($res['userid'])?'<option value="6">退款</option>':null).'<option value="5">删除订单</option></select></td></tr>';
}
?>
          </tbody>
        </table>
