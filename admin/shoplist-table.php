...
if(isset($_GET['kw'])){
	...
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools A where{$sql}");
	...
	$sql=" prid='$prid'";
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools where{$sql}");
	...
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools");
	$sql=" 1";
...
?>
	  ...
	  <div class="table-responsive">
        <table class="table table-striped" id="shoplist">
          <thead><tr><th>商品ID</th><th>商品名称</th><th>商品添加时间</th><th>商品价格设置</th><th>商品类型</th><th class="<?php echo isset($_GET['cid'])?'hide':'';?>">所属分类</th><th>排序操作</th><th>库存</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
