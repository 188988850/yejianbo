...
if(isset($_GET['kw'])){
	...
	$sql=" B.name LIKE '%$kw%'";
	$numrows=$DB->getColumn("SELECT count(*) FROM shua_inviteshop A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE{$sql}");
	$con='包含 <b>'.$kw.'</b> 的共有 <b>'.$numrows.'</b> 个商品';
	$link='&kw='.$kw;
}elseif(isset($_GET['id'])){
	$id = intval($_GET['id']);
	$numrows=$DB->getColumn("SELECT count(*) from shua_inviteshop where id='$id'");
	$sql=" id='$id'";
	$con='推广商品列表';
	$link='&id='.$id;
}else{
	$numrows=$DB->getColumn("SELECT count(*) from shua_inviteshop");
	$sql=" 1";
...
$rs=$DB->query("SELECT A.*,B.name FROM shua_inviteshop A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE{$sql} ORDER BY A.sort ASC LIMIT $offset,$pagesize");
while($res = $rs->fetch())
{
	if($res['type']==1){
