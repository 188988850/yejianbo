...
{
$id=$_GET['id'];
$row=$DB->getRow("select * from shua_message where id='$id' limit 1");
?>
<div class="block">
...
$rows=$DB->getRow("select * from shua_message where type='$type' and title='$title' limit 1");
if($rows)
	showmsg('通知标题已存在！',3);
$sql="insert into `shua_message` (`type`,`title`,`content`,`addtime`,`active`) values ('".$type."','".$title."','".$content."','".$date."','1')";
...
{
$id=$_GET['id'];
$rows=$DB->getRow("select * from shua_message where id='$id' limit 1");
if(!$rows)
	showmsg('当前记录不存在！',3);
$title=daddslashes($_POST['title']);
$type=intval($_POST['type']);
...
if($DB->exec("update shua_message set type='$type',title='$title',content='$content' where id='{$id}'")!==false)
...
{
$numrows=$DB->getColumn("SELECT count(*) from shua_message");
?>
<div class="block">
...
$rs=$DB->query("SELECT * FROM shua_message WHERE 1 order by id desc limit $offset,$pagesize");
while($res = $rs->fetch())
{
