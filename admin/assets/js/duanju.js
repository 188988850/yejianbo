...
function listTable(query){
	...
	if(query == 'start' || query == undefined){
		query = '';
		history.replaceState({}, null, './duanju.php');
	}else if(query != undefined){
		history.replaceState({}, null, './duanju.php?'+query);
	}
	layer.closeAll();
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'GET',
		url : 'duanju-table.php?num='+pagesize+'&'+query,
		dataType : 'html',
		cache : false,
...
function editItem(tid) {
    ...
    nameLabel.innerHTML = '短剧名称：';
    var nameInput = document.createElement('input');
    nameInput.type = 'text';

    var descriptionLabel = document.createElement('label');
    descriptionLabel.innerHTML = '短剧链接：';
    ...
    submitButton.onclick = function() {
        ...
        var input444 = descriptionInput.value;

        // 创建 XMLHttpRequest 对象
        var xhr = new XMLHttpRequest();

        // 准备发送请求
        xhr.open('POST', 'ajax_class.php?act=editClass222&tid=' + tid, true);

        // 设置请求头，如果需要
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');

        // 发送请求
        xhr.send('tid=' + tid + '&name=' + name + '&input444=' + input444);
...
function setTools3(tid,close) {
	$.ajax({
		type : 'GET',
		url : 'ajax_shop.php?act=zhiding&tid='+tid+'&close='+close,
		dataType : 'json',
		success : function(data) {
			listTable();
		},
...
function setTools2(tid,close) {
	$.ajax({
		type : 'GET',
		url : 'ajax_shop.php?act=setTools2&tid='+tid+'&close='+close,
		dataType : 'json',
		success : function(data) {
			listTable();
		},
...
function zhiding(tid) {
	var confirmobj = layer.confirm('你确实要置顶此短剧吗？', {
	  btn: ['确定','取消']
	}, function(){
	  $.ajax({
		type : 'GET',
		url : 'ajax_shop.php?act=zhiding&tid='+tid,
		dataType : 'json',
		success : function(data) {
			if(data.code == 0){
				layer.msg('置顶成功');
...
function delTool(tid) {
	var confirmobj = layer.confirm('你确实要删除此短剧吗？', {
	  btn: ['确定','取消']
	}, function(){
	  $.ajax({
		type : 'GET',
		url : 'ajax_shop.php?act=delTool2&tid='+tid,
		dataType : 'json',
		success : function(data) {
			if(data.code == 0){
				layer.msg('删除成功');
