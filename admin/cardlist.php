...
for ($i = 0; $i < $num; $i++) {
	$km=getkm(18);
	$sql=$DB->exec("insert into `shua_kms` (`type`,`km`,`tid`,`num`,`addtime`) values (1,'".$km."','".$tid."','".$value."','".$date."')");
	if($sql) {
...
<div class="block-title w h"><h3 class="panel-title">清空卡密</h3></div>
<div class=" box">';
if($DB->exec("DELETE FROM shua_kms WHERE type=1")!==false){
...
<div class="block-title w h"><h3 class="panel-title">清空卡密</h3></div>
<div class=" box">';
if($DB->exec("DELETE FROM shua_kms WHERE type=1 AND status=1")!==false){
echo '<div class="box">清空成功.</div>';
}else{
echo'<div class="box">清空失败.</div>';
}
...
$rs=$DB->query("SELECT A.*,B.name FROM shua_kms A LEFT JOIN shua_tools B ON A.tid=B.tid WHERE{$sql} order by kid desc limit $offset,$pagesize");
while($res = $rs->fetch())
{
if($res['status']==0) {
