		...
		if($_POST['type']==1){
			if(isset($_POST['tid'])){
				...
				foreach($arr['class'] as $row){
					...
					if($srow=$DB->getRow("select cid from shua_class where name='{$row['name']}' limit 1")){
						$classa[$row['cid']] = $srow['cid'];
					}else{
						$id = $DB->exec("INSERT INTO `shua_class` (`name`,`sort`,`active`,`shopimg`) VALUES ('".$row['name']."','".$row['sort']."','".$row['active']."','".$row['shopimg']."')");
				...
				foreach($arr['shequ'] as $row){
					...
					if($srow=$DB->getRow("select id from shua_shequ where url='{$row['url']}' limit 1")){
						$shequa[$row['id']] = $srow['id'];
					}else{
						$id = $DB->exec("INSERT INTO `shua_shequ` (`url`,`username`,`password`,`type`) VALUES ('".$row['url']."',NULL,NULL,'".$row['type']."')");
				...
				foreach($arr['price'] as $row){
					...
					if($srow=$DB->getRow("select id from shua_price where kind='{$row['kind']}' and name='{$row['name']}' limit 1")){
						$pricea[$row['id']] = $srow['id'];
					}else{
						$id = $DB->exec("INSERT INTO `shua_price` (`kind`,`name`,`p_0`,`p_1`,`p_2`) VALUES ('".$row['kind']."','".$row['name']."','".$row['p_0']."','".$row['p_1']."','".$row['p_2']."')");
				...
				foreach($arr['tools'] as $row){
					if(!in_array($row['tid'],$_POST['tid']))continue;
					if(!$DB->getRow("select tid from shua_tools where name='{$row['name']}' and is_curl='{$row['is_curl']}' limit 1")){
						$DB->exec("INSERT INTO `shua_tools` (`cid`,`name`,`price`,`cost`,`cost2`,`prid`,`prices`,`input`,`inputs`,`desc`,`alert`,`shopimg`,`value`,`is_curl`,`curl`,`shequ`,`goods_id`,`goods_type`,`goods_param`,`repeat`,`multi`,`valiserv`,`min`,`max`,`validate`,`sort`,`close`,`active`) VALUES ('".$classa[$row['cid']]."','".$row['name']."','".$row['price']."','".$row['cost']."','".$row['cost2']."','".$pricea[$row['prid']]."','".$row['prices']."','".$row['input']."','".$row['inputs']."','".addslashes($row['desc'])."','".addslashes($row['alert'])."','".$row['shopimg']."','".$row['value']."','".$row['is_curl']."','".$row['curl']."','".$shequa[$row['shequ']]."','".$row['goods_id']."','".$row['goods_type']."','".$row['goods_param']."','".$row['repeat']."','".$row['multi']."','".$row['min']."','".$row['max']."','".$row['validate']."','".$row['valiserv']."','".$row['sort']."','".$row['close']."','".$row['active']."')");
						$success++;
					}
				}
...
<?php
		...
		$DB->exec("TRUNCATE TABLE `shua_class`");
		$success++;
		foreach($arr['class'] as $row){
			$DB->exec("INSERT INTO `shua_class` (`cid`,`name`,`sort`,`active`,`shopimg`) VALUES ('".$row['cid']."','".$row['name']."','".$row['sort']."','".$row['active']."','".$row['shopimg']."')");
			$success++;
		}
		$DB->exec("TRUNCATE TABLE `shua_tools`");
		$success++;
		foreach($arr['tools'] as $row){
			$DB->exec("INSERT INTO `shua_tools` (`tid`,`cid`,`name`,`price`,`cost`,`cost2`,`prid`,`prices`,`input`,`inputs`,`desc`,`alert`,`shopimg`,`value`,`is_curl`,`curl`,`shequ`,`goods_id`,`goods_type`,`goods_param`,`repeat`,`multi`,`min`,`max`,`validate`,`valiserv`,`sort`,`close`,`active`) VALUES ('".$row['tid']."','".$row['cid']."','".$row['name']."','".$row['price']."','".$row['cost']."','".$row['cost2']."','".$row['prid']."','".$row['prices']."','".$row['input']."','".$row['inputs']."','".addslashes($row['desc'])."','".addslashes($row['alert'])."','".$row['shopimg']."','".$row['value']."','".$row['is_curl']."','".$row['curl']."','".$row['shequ']."','".$row['goods_id']."','".$row['goods_type']."','".$row['goods_param']."','".$row['repeat']."','".$row['multi']."','".$row['min']."','".$row['max']."','".$row['validate']."','".$row['valiserv']."','".$row['sort']."','".$row['close']."','".$row['active']."')");
			$success++;
		}
		$DB->exec("TRUNCATE TABLE `shua_shequ`");
		$success++;
		foreach($arr['shequ'] as $row){
			$DB->exec("INSERT INTO `shua_shequ` (`id`,`url`,`username`,`password`,`type`) VALUES ('".$row['id']."','".$row['url']."',NULL,NULL,'".$row['type']."')");
			$success++;
		}
		$DB->exec("TRUNCATE TABLE `shua_price`");
		$success++;
		foreach($arr['price'] as $row){
			$DB->exec("INSERT INTO `shua_price` (`id`,`kind`,`name`,`p_0`,`p_1`,`p_2`) VALUES ('".$row['id']."','".$row['kind']."','".$row['name']."','".$row['p_0']."','".$row['p_1']."','".$row['p_2']."')");
			$success++;
		}
		$CACHE->clear('pricerules');
