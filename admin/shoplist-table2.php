...
if(isset($_GET['kw'])){
	...
	$sql=" A.active LIKE '%$kw%' AND A.close = 1";
	if(is_numeric($kw))$sql.=" OR A.active='$kw'";
...
$numrows = $DB->getColumn("SELECT count(*) FROM shua_tools WHERE close = 1");
	$sql=" 1";
	$con='已下架商品管理';
}
?>
...
$extraCondition = " AND close = 1";
$rs = $DB->query("SELECT A.*,B.name classname FROM shua_tools A LEFT JOIN shua_class B ON A.cid=B.cid WHERE {$sql}{$extraCondition} order by {$orderby} limit $offset,$pagesize");
while($res = $rs->fetch())
{
if($res['is_curl']==4){
	$stock = '发卡';
