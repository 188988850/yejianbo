...
if($_POST['tgprice']){
    	$tid=intval($_POST['tgprice']);
   		  $DB->exec("update shua_config set v='{$_POST['tgprice']}' where k='tgprice'");
}
...
if($_GET['cg']){
    	$tid=intval($_GET['cg']);
    		...
    		$DB->exec("update shua_tools set close='0' where tid='{$tid}'");
}
if($_GET['close']){
    	$tid=intval($_GET['close']);
    		$DB->exec("update shua_tools set close='1' where tid='{$tid}'");
}
if($_GET['jj']){
    	$tid=intval($_GET['jj']);
    		$DB->exec("update shua_tools set close='88' where tid='{$tid}'");
    		$DB->exec("update shua_tools set active='0' where tid='{$tid}'");
}
if($_GET['fb']){
    	$tid=intval($_GET['fb']);
    		$DB->exec("update shua_tools set close='1' where tid='{$tid}'");
    		$DB->exec("update shua_tools set active='1' where tid='{$tid}'");
}
if($_GET['sc']){
    	$tid=intval($_GET['sc']);
    		$DB->exec("DELETE FROM shua_orders WHERE tid='$tid'");
}
$shequlist=$DB->getAll("SELECT id,url FROM shua_shequ order by id asc");
$shequurls=[];
...
$classlist=$DB->getAll("SELECT * FROM shua_class WHERE active=1 order by sort asc");
...
if($_SESSION['price_class']){
	...
	$pricelist=$DB->getAll("SELECT * FROM shua_price order by id asc");
	$price_class[0]='不加价';
...
$tgprice = $DB->getRow("select * from shua_config where k='tgprice' limit 1");

$pagesize = isset($_GET['num'])?intval($_GET['num']):30;
$orderby = 'A.tid desc';
	$numrows=$DB->getColumn("SELECT count(*) from shua_tools WHERE pro>'0'");
	$sql=" pro>'0'";
...
if(isset($_POST['tcprice'])){
  ...
  $tcprice=$_POST['tcprice'];
  	$DB->exec("update shua_tools set tc='{$tcprice}' where tid='{$spid}'");
}
if($_GET['tc']){
    $spid=$_GET['tc'];
	$row=$DB->getRow("select * from shua_tools where tid='$spid' limit 1");
...
<h2>用户共投稿 <b><?php echo $numrows ?></b> 个商品</h2></div>
	  ...
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter orderList">
          <thead><tr><th>商品ID</th><th>用户ZID</th><th>商品名称</th><th>商品价格</th><th>商品提成</th><th>资源链接</th><th>投稿人微信号</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
$rs=$DB->query("SELECT A.*,B.name classname FROM shua_tools A LEFT JOIN shua_class B ON A.cid=B.cid WHERE{$sql} order by {$orderby} limit $offset,$pagesize");
while($res = $rs->fetch())
{
...
echo '<tr><td><a href="./shoplist.php?tid='.$res['tid'].'" target="_blank">'.$res['tid'].'</a></td><td><a href="./sitelist.php?zid='.$res['pro'].'" target="_blank">'.$res['pro'].'</a></td><td>'.$res['name'].'</td>'.($res['prid']>0?'<td><span onclick="getPrice('.$res['tid'].')"><font color="blue">'.$price_class[$res['prid']].'</font>&nbsp;(成本:'.$res['price'].')</span></td>':'<td><span onclick="getPrice('.$res['tid'].')">'.$res['price'].'｜'.$res['cost'].'｜'.$res['cost2'].'</span>
</td>').'<td>'.$res['tc'].'元</td><td>'.$res['zyurl'].'</td><td>'.$res['wx_test'].'</td><td>'.($res['close']==1?'
    <a class="btn btn-xs btn-warning" href="?cg='.$res['tid'].'">未通过</a>':'
    <a class="btn btn-xs btn-success" href="?close='.$res['tid'].'">上架中</a>').''.$susu.'</td><td><a href="./shopedit.php?my=edit&tid='.$res['tid'].'" class="btn btn-info btn-xs" target="_blank">编辑</a>&nbsp;<a href="?
tc='.$res['tid'].'" class="btn btn-xs btn-info" target="_blank">提成</a>&nbsp;<a href="?
