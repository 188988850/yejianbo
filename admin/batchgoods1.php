...
<div class="col-sm-12 col-md-10 center-block" style="float: none;">
    ...
    $rs = $DB->query("SELECT `cid`,`name` FROM shua_class WHERE active=1 order by sort asc");
    $classSelect = '<option value="0">不进行分类</option>';
    while ($res = $rs->fetch()) {
        $classSelect .= '<option value="' . $res['cid'] . '">' . $res['name'] . '</option>';
    }
    unset($rs);
    $rs = $DB->query('SELECT `id`,`type`,`url`,`remark` FROM shua_shequ order by id asc');
    $shequselect = '';
    while ($res = $rs->fetch()) {
    ...
    unset($rs);
    $rs = $DB->query("SELECT `id`,`kind`,`p_0`,`p_1`,`p_2`,`name` FROM shua_price order by id asc");
