     ...
     <div class="block-title"><h2>社区/卡盟对接日志</h2></div>
      <div class="table-responsive">
        <table class="table table-striped">
          <tbody>
<?php
$numrows=$DB->getColumn("SELECT count(*) from shua_logs WHERE action='社区对接' OR action='自动访问URL'");
$pagesize=30;
$pages=ceil($numrows/$pagesize);
$page=isset($_GET['page'])?intval($_GET['page']):1;
$offset=$pagesize*($page - 1);

$rs=$DB->query("SELECT * FROM shua_logs WHERE action='社区对接' OR action='自动访问URL' order by id desc limit $offset,$pagesize");
while($res = $rs->fetch())
{
