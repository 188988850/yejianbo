...
elseif($act=='orders')
{
	...
	if($key!=$conf['apikey'])exit('{"code":-4,"msg":"API对接密钥错误，请在后台设置密钥"}');
	if($tid){
		$tool=$DB->getRow("SELECT tid,value FROM shua_tools WHERE tid='$tid' AND active=1 LIMIT 1");
	...
	while($res = $rs->fetch()){
		...
		if($_GET['sign']==1)$DB->exec("UPDATE `shua_orders` SET status=1 WHERE `id`='{$res['id']}'");
	}
	if($format=='text'){
		$txt = '';
		foreach($data as $row){
...
elseif($act=='change')
{
	...
	if($key!=$conf['apikey'])exit('{"code":-4,"msg":"API对接密钥错误，请在后台设置密钥"}');
	$row=$DB->getRow("SELECT id FROM shua_orders WHERE id='$id' LIMIT 1");
	if($row) {
...
elseif($act == 'goodslistbycid')
{
	if(isset($_POST['user']) && isset($_POST['pass'])){
		...
		$userrow = $DB->getRow("SELECT * FROM `shua_site` WHERE `user` = '{$user}' LIMIT 1");
...
elseif($act == 'getleftcount')
{
	...
	if(isset($tids) && count($tids)>0){
		$data = [];
		foreach($tids as $tid){
			$tool = $DB->getRow("SELECT * FROM `shua_tools` WHERE `tid` = ".intval($tid)." LIMIT 1");
			if(!$tool)continue;
			if($tool['is_curl']==4){
...
elseif($act == 'pay')
{
	...
	if ($tool && $tool['active'] == 1) {
		...
		if ($userrow && $userrow['user'] == $user && $userrow['pwd'] == $pass && $userrow['status'] == 1) {
			...
			$sql="INSERT INTO `shua_pay` (`trade_no`,`type`,`tid`,`zid`,`input`,`num`,`name`,`money`,`ip`,`userid`,`addtime`,`blockdj`,`status`) VALUES (:trade_no, :type, :tid, :zid, :input, :num, :name, :money, :ip, :userid, NOW(), :blockdj, 0)";
			...
			if ($DB->exec($sql, $data)) {
				if ($DB->exec("UPDATE `shua_site` SET `rmb` = `rmb` - {$need} WHERE `zid` = '{$userrow['zid']}'") && $DB->exec("UPDATE `shua_pay` SET `status` = 1 WHERE `trade_no` = '{$trade_no}'")) {
					...
					if($orderid = processOrder($srow)){
						...
						$djzt = $DB->getColumn("SELECT djzt FROM shua_orders WHERE id = '$orderid' LIMIT 1");
						if($djzt == 3){
...
elseif($act=='duanju')
{
    // 假设 $DB 是数据库连接对象或者数据库操作类的实例
    $rows = $DB->getAll("SELECT * FROM shua_duanju");

    // 设置响应头为 JSON 格式
    header('Content-Type: application/json');

    // 构建 API 返回的 JSON 数据格式
    $responseData = [];
    foreach ($rows as $row) {
        $responseData[] = [
            'name' => $row['name'],
...
elseif($act=='siteinfo')
{
	...
	$count3=$DB->getColumn("SELECT count(*) from shua_site");
