...
if($userrow['power']==2){
	...
	if($userrow['upzid']>1){
		$ktfz_price2 = $DB->getColumn("SELECT ktfz_price2 FROM shua_site WHERE zid='{$userrow['upzid']}' LIMIT 1");
	...
	if($userrow['upzid']>1){
		$ktfz_price = $DB->getColumn("SELECT ktfz_price FROM shua_site WHERE zid='{$userrow['upzid']}' LIMIT 1");
...
if($_GET['act']=='submit'){
	...
	$DB->exec("UPDATE `shua_site` SET `endtime`='$endtime',`rmb`=`rmb`-'{$price}' WHERE `zid`='{$userrow['zid']}'");
