...
if(isset($_GET['act']) && $_GET['act']=='qrlogin'){
	...
	if(isset($_SESSION['findpwd_qq']) && $qq=$_SESSION['findpwd_qq']){
		$row=$DB->getRow("SELECT zid,user,pwd,status FROM shua_site WHERE qq=:qq LIMIT 1", [':qq'=>$qq]);
		...
		if($row['user']){
			...
			$DB->exec("UPDATE shua_site SET lasttime='$date' WHERE zid='{$row['zid']}'");
	...
	$row=$DB->getRow("SELECT * FROM shua_site WHERE {$typecolumn}='{$openid}' limit 1");
	if($row){
		$user=$row['user'];
		$pass=$row['pwd'];
		if($islogin2==1){
		...
		$sds=$DB->exec("update `shua_site` set `{$typecolumn}`='$openid' where `zid`='{$userrow['zid']}'");
		@header('Content-Type: text/html; charset=UTF-8');
