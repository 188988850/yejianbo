...
<form action="./kscxdd.php" method="GET" class="form-inline">
  ...
  <div>
	<select name="type" class="form-control"><option value="-1">全部状态</option><option value="0">待处理</option><option value="2">处理中</option><option value="3">有异常</option><option value="4">已退款</option><option value="1">已完成</option></select>
...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	$numrows=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE{$sql}");
	$con='
...
$numrows=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE{$sql} AND userid='{$user_id}'");
		$ondate=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE status=1 AND{$sql} AND userid='{$user_id}'");
		$ondate2=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE status=2 AND{$sql} AND userid='{$user_id}'");
		$ondate3=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE status=3 AND{$sql} AND userid='{$user_id}'");
		$ondate4=$DB->getColumn("SELECT count(*) FROM shua_orders A WHERE status=4 AND{$sql} AND userid='{$user_id}'");
		$con='
    <div style="background: #f2f2f2; height: 10px"></div>
...
$rs=$DB->query("SELECT A.*,B.name FROM shua_orders A left join shua_tools B on A.tid=B.tid WHERE{$sql} ORDER BY id DESC LIMIT $offset,$pagesize");
while($res = $rs->fetch())
{
