...
// 获取下级分站数量
$today = date('Y-m-d');
$numrows = $DB->getColumn("SELECT count(*) FROM shua_site WHERE upzid='{$userrow['zid']}' AND (power=2) AND DATE(addtime)='{$today}'");
...
$sql = "SELECT SUM(reward) FROM shua_qiandao66 WHERE zid='{$userrow['zid']}'";
$sum = $DB->getColumn($sql);
$day = date("Y-m-d");
$lastday = date("Y-m-d",strtotime("-1 day"));
$ggu = $conf['jiangli'];
if ($row = $DB->getRow("SELECT * FROM shua_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$day' AND reward=$ggu ORDER BY id DESC LIMIT 1")) {
	$isqiandao3 = true;
...
$sql = "SELECT SUM(reward) FROM shua_qiandao66 WHERE zid=zid";
$sum = $DB->getColumn($sql);
$day = date("Y-m-d");
$ggu = $conf['jiangli2'];
$lastday = date("Y-m-d",strtotime("-1 day"));
...
$lastday = date("Y-m-d",strtotime("-1 day"));
$ggu = $conf['jiangli3'];
if ($row = $DB->getRow("SELECT * FROM shua_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$day' AND reward=15 ORDER BY id DESC LIMIT 1")) {
	$isqiandao2 = true;
	$continue = $row['continue'];
}else{
	if ($row = $DB->getRow("SELECT * FROM shua_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$day' AND reward=15 ORDER BY id DESC LIMIT 1")) {
...
$today = date("Y-m-d");
$sql = "SELECT SUM(reward) FROM shua_qiandao66 WHERE zid='{$userrow['zid']}' AND date='$today'";
