...
if($_GET['act']=='submit'){
	...
	elseif ($domain2 == $userrow['domain']) {
		showmsg('不能与主域名一样！',3);
	}
	elseif ($userrow['power'] > 0){
		// 执行数据库查询，获取 domain 和 domain2 字段的所有值，并将结果存储在 $result 变量中
$result = $DB->getAll("SELECT domain, domain2 FROM shua_site");

// 遍历查询结果
foreach ($result as $row) {
    // 如果当前行的 domain 或 domain2 等于变量 $domain2
    if ($row['domain'] == $domain2 || $row['domain2'] == $domain2) {
        // 显示提示信息 '此前缀已被使用！'，并重定向到页面 3
        showmsg('此域名已被使用，请更换！', 3);
        break;  // 找到匹配后，停止循环
    }
}
	}
...
elseif ($DB->getRow("SELECT zid FROM shua_site WHERE domain=:domain OR domain2=:domain LIMIT 1", [':domain'=>$domain]) || $qz=='www' || $domain==$_SERVER['HTTP_HOST'] || in_array($domain,explode(',',$conf['fenzhan_remain']))) {
		showmsg('此前缀已被使用，请更换！',3);
	}
	
	
	
	if($price>$userrow['rmb'])exit("<script language='javascript'>alert('你的余额不足，请充值！');window.location.href='./';</script>");
	$DB->exec("UPDATE `shua_site` SET `domain2`=:domain2,`rmb`=`rmb`-:rmb WHERE `zid`=:zid", [':domain2'=>$domain2, ':rmb'=>$price, ':zid'=>$userrow['zid']]);
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 80px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/genghuan.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>更换备用域名</span></a>
