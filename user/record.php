...
$outcome_lastday=$DB->getColumn("SELECT sum(point) FROM shua_points WHERE zid='{$userrow['zid']}' AND action='消费' AND addtime<'$thtime' AND addtime>'$lastday'");
?>
<!DOCTYPE html>
<html lang="zh-cn">
<head>
...
<style>
    ...
    .hotxy .hotxy-item-index{
        border-bottom: 3px solid #ffa56a;
        font-weight: 700;
...
<div class="main-content">
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 80px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/sztj.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>收支统计</span></a>
                    <a style="font-size: 14px; color: rgb(255, 91, 33); padding-left: 11px;"><span><span></span></a>
                    <div class="charge-btn"><a href="kscxsy.php" style="color: #fff;">快速查看明细</a></div></div>
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 20px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/szmx.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>收支明细</span></a>
    ...
    <div class="hotxy block-white" id="tab">
            ...
            <a class="hotxy-item <?php if($action=="转账") echo "hotxy-item-index" ?>" id="tab_3" href="./record.php?action=转账">转账</a>
    </div>
...
$sql = "SELECT * FROM shua_points WHERE zid= ".$userrow['zid'];
$sqlCount = "SELECT count(*) from shua_points WHERE zid='{$userrow['zid']}'";
if ($action) {
...
while($res = $rs->fetch())
           {
             if($res['action']=='提成'){$cc='+';$ys='item-logo-2';}//蓝
               elseif($res['action']=='赠送'){$cc='+';$ys='item-logo-3';}//红
                elseif($res['action']=='加款'){$cc='+';$ys='item-logo-9';}//红
                 elseif($res['action']=='充值'){$cc='+';$ys='item-logo-9';}//红
                  elseif($res['action']=='退款'){$cc='+';$ys='item-logo-8';}//灰
                   elseif($res['action']=='消费'){$cc='-';$ys='item-logo-1';}//黄
                    elseif($res['action']=='提现'){$cc='-';$ys='item-logo-3';}//红
                     elseif($res['action']=='转账'){$cc='';$ys='item-logo-2';}//蓝
                      elseif($res['action']=='扣除'){$cc='-';$ys='item-logo-8';}//灰
                      elseif($res['action']=='奖励'){$cc='+';$ys='item-logo-3';}//灰

                echo 
            '
      <div class="list-item">
                         <div class="list-item-top" style="padding-bottom: 10px">
                              <div class="'.$ys.'" style="width: auto;padding-right: 10px">
                                 <div class="item-logo-img"style="width: auto;padding: 0 30px;font-size: 13px;">
                                 '.$res['action'].'
                                 </div>
                              </div>
                         </div>
