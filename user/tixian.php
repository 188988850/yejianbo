...
<body><style>
   ...
   //  获取今天开始时间
    $beginToday=date("Ymd");
    $points = $DB->getRow("SELECT * FROM shua_points WHERE zid='{$userrow['zid']}' and action ='提现' and DATE_FORMAT(addtime,'%Y%m%d')=$beginToday");
    if($points){
    ...
    $sql = "INSERT INTO `shua_tixian` (`zid`, `money`, `realmoney`, `pay_type`, `pay_account`, `pay_name`, `status`, `addtime`) VALUES (:zid, :money, :realmoney, :pay_type, :pay_account, :pay_name, 0, NOW())";
    ...
    if($DB->exec($sql, $data)){
    	if($DB->exec("UPDATE shua_site SET rmb=rmb-:money,rmbtc=rmbtc-:money WHERE zid=:zid", [':money'=>$money, ':zid'=>$userrow['zid']])){
    ...
    $numrows=$DB->getColumn("SELECT count(*) FROM shua_tixian WHERE zid='{$userrow['zid']}'");
        ...
        <div style="position: relative;width: 100%;background-color:#f2f2f2;  padding-bottom: 30px;">
            ...
            <div style="width: 100%;position: 10px ;min-height: 22vh;position: relative">
                <div class="block-back" style="padding: 11px 15px">
                    <a href="tixiansq.php" class="font-weight display-row align-center">
...
<!--返回首页-->
                    ...
                    <?php }?>                   
                        ...
                        <div style="height: 4rem;width: 100%;text-align: center;line-height: 4rem" class="form-group-border-top">
                            <a style="color: #ff0000;font-size: 1.3rem;">仅支持推广获得的佣金可进行提现</a>
                        </div>
                    </div>
                    <div style="height:3.7rem;width: 95%; margin: 10px auto; border-radius: 50px; background: #fff; font-size: 1.1rem;" class="display-row align-center">
                        <font style="font-weight: 550;margin:0 10px"></font>
                        <input type="number" name="money"
                               placeholder="输入提现金额,金额只能为整数"
        ...
        <div class="my-cell block-white form-group-border-bottom" style="margin-bottom: 0px;border-radius: 0;padding: 2px 10px">
            <div class="my-cell-title display-row justify-between align-center">
                ...
                <div class="my-cell-title-r  display-row  align-center">
                    <span onclick="openmsg1()" style="color: #939393;font-size:1.3rem">提现规则 <i class="fa fa-angle-right" style="padding-left: 5px"></i></span>
                </div>
            </div>

        <?php
    $rs=$DB->query("SELECT * FROM shua_tixian WHERE zid='{$userrow['zid']}' ORDER BY id DESC LIMIT 100");
    while($res = $rs->fetch())
        {
          echo '
