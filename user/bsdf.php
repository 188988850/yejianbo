...
if(isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $row = $DB->getRow("SELECT * FROM shua_sscc WHERE id=:id AND zid=:zid LIMIT 1", [':id'=>$id, ':zid'=>$userrow['zid']]);
    if(!$row) {
        exit("<script language='javascript'>alert('群聊不存在！');history.go(-1);</script>");
    }
} else {
    exit("<script language='javascript'>alert('未获取到ID参数！');history.go(-1);</script>");
}
        ...
        <div class="content-item">
            <div class="content-item-top">
                ...
                <div style="padding: 5px 0;font-size: 1.3rem;color: #858585;">
                    ...
                    <font color="#000"><a href="<?php echo $conf['cosqun01'] ?>?zid=<?php echo $zid?>&id=<?php echo $row['id']?>" target="_blank" rel="noreferrer"><?php echo $conf['cosqun01'] ?>?zid=<?php echo $zid?>&id=<?php echo $row['id']?></a></font>
                    <a style="width: 100%;" href="javascript:;" id="copy-btn1" data-clipboard-text="<?php echo $conf['cosqun01'] ?>?zid=<?php echo $zid?>&id=<?php echo $row['id']?>">
        ...
        <div class="layui-carousel" id="test1" lay-filter="test1" style="margin: 25px auto; width: 95%; height: 50vh;" lay-anim="" lay-indicator="outside" lay-arrow="always">
            <div carousel-item="" class="text-center">
                <div class="layui-this">
                    <img style=" height: 50vh" id="img_1" src="./timg/qunliao.php?id=1&url=<?php echo urlencode($conf['cosqun01'] . '?zid=' . $zid . '&id=' . $row['id']); ?>" alt="推广图1"></div>
        </div>
    </div>
       </div>
<?php }?>
