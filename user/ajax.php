...
case 'login':
	...
	$row=$DB->getRow("SELECT * FROM shua_site WHERE user=:user LIMIT 1", [':user'=>$user]);
	if($row && $user===$row['user'] && $pass===$row['pwd']) {
		...
		if($_SESSION['Oauth_qq_openid'] && $_SESSION['Oauth_qq_token']){
			...
			$DB->exec("UPDATE shua_site SET {$typecolumn}=:qq_openid,lasttime=NOW() WHERE zid=:zid", [':qq_openid'=>$_SESSION['Oauth_qq_openid'], ':zid'=>$row['zid']]);
			...
			$DB->exec("UPDATE shua_site SET lasttime=NOW() WHERE zid=:zid", [':zid'=>$row['zid']]);
...
case 'unbind':
	...
	if($DB->exec("UPDATE shua_site SET {$typecolumn}=NULL WHERE zid='{$userrow['zid']}'")){
...
case 'quickreg':
	...
	$sql="INSERT INTO shua_site (`upzid`,`power`,`domain`,`domain2`,`user`,`pwd`,`{$typecolumn}`,`nickname`,`faceimg`,`rmb`,`qq`,`sitename`,`keywords`,`description`,`addtime`,`lasttime`,`status`) VALUES (:upzid,0,NULL,NULL,:user,:pwd,:qq_openid,:nickname,:faceimg,'0',NULL,NULL,NULL,NULL,NOW(),NOW(),'1')";
	...
	if($DB->exec($sql, $data)){
		...
		unset($_SESSION['Oauth_qq_nickname']);
		unset($_SESSION['Oauth_qq_faceimg']);
		$DB->exec("UPDATE shua_orders SET userid='".$zid."' WHERE userid='".$cookiesid."'");
...
case 'checkdomain':
	...
	$srow=$DB->getRow("SELECT zid FROM shua_site WHERE domain=:domain OR domain2=:domain LIMIT 1", [':domain'=>$domain]);
	if($srow)exit('1');
	else exit('0');
break;
case 'checkuser':
	$user = trim($_GET['user']);
	$srow=$DB->getRow("SELECT zid FROM shua_site WHERE user=:user LIMIT 1", [':user'=>$user]);
	if($srow)exit('1');
	else exit('0');
break;
case 'reguser':
	...
	$sql="INSERT INTO shua_site (`upzid`,`power`,`domain`,`domain2`,`user`,`pwd`,`rmb`,`qq`,`sitename`,`keywords`,`description`,`anounce`,`bottom`,`modal`,`addtime`,`lasttime`,`status`) VALUES (:upzid,0,NULL,NULL,:user,:pwd,'0',:qq,NULL,NULL,NULL,NULL,NULL,NULL,NOW(),NOW(),'1')";
...
while($upzid > 0) {
	...
	if($need==0){
		...
		if($islogin2==1){
			$sql="UPDATE shua_site SET power=:power,domain=:domain,sitename=:sitename,title=:title,keywords=:keywords,description=:description,kfqq=`qq`,endtime=:endtime WHERE zid=:zid";
			...
			$sql="INSERT INTO shua_site (`upzid`,`power`,`domain`,`domain2`,`user`,`pwd`,`rmb`,`qq`,`sitename`,`title`,`keywords`,`description`,`kfqq`,`addtime`,`endtime`,`status`) VALUES (:upzid, :power, :domain, NULL, :user, :pwd, :rmb, :qq, :sitename, :title, :keywords, :description, :kfqq, NOW(), :endtime, 1)";
		...
		if($zid){
			$_SESSION['newzid']=$zid;
			unset($_SESSION['addsalt']);
			if(!$islogin2)$DB->exec("UPDATE shua_orders SET userid='".$zid."' WHERE userid='".$cookiesid."'");
			$DB->exec("UPDATE shua_orders SET zid='".$zid."' WHERE userid='".$zid."'");
...
break;
case 'thing':
	$rs=$DB->query("SELECT * FROM shua_tools WHERE `pro`='{$userrow['zid']}' order by addtime desc");
	$data = array();
	while($res = $rs->fetch()){
