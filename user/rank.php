...
if($_GET['last']==1){
	$sql = "select a.zid,(select b.sitename from shua_site as b where a.zid=b.zid) as sitename,count(id) as count,sum(money) as money from shua_orders as a where addtime>'$lastday' and addtime<'$thtime' and zid>1 and `status` != 4 group by zid order by money desc limit {$limit}";
	$addstr = '已发放奖励';
}else{
	$sql = "select a.zid,(select b.sitename from shua_site as b where a.zid=b.zid) as sitename,count(id) as count,sum(money) as money from shua_orders as a where addtime>'$thtime' and zid>1 and `status` != 4 group by zid order by money desc limit {$limit}";
	$addstr = '预计发放奖励';
}
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 80px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/paihangbang.png" style="width: 25px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>排行榜详情</span></a>
                    <a style="font-size: 14px; color: rgb(255, 91, 33); padding-left: 11px;"><span></span></a>
                    </div>
                    <div class="content-item-bottom">

<ul class="nav nav-pills">
<li class="<?php echo $_GET['last']!=1?'active':null;?>" style="width:49.5%"><a href="rank.php" style="font-size: 13px;"><center>今日销售排行</center></a></li>
<li class="<?php echo $_GET['last']==1?'active':null;?>" style="width:49.5%"><a href="rank.php?last=1" style="font-size: 13px;"><center>昨日销售排行</center></a></li>
</ul>

            <br>
          <tbody><br><br><br>
              <div style="text-align:center;color: #838383;">- 暂未公布排行榜信息 -</div><br><br><br>
<!--<?php
$rs=$DB->query($sql);
$i=1;
while($res = $rs->fetch())
{
    echo '<div class="list-item">
                                 <div class="list-item-top">
                                      <div class="item-logo-7" style="width: auto;padding-right: 8px">
