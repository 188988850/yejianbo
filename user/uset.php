...
<div class="main-content">
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 80px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/ziliao.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>个人信息</span></a>
                    ...
                    <div class="form-group form-group-transparent form-group-border-bottom">
                        <div class="input-group" style="width:100%">
                            <div class="input-group-addon">
                                联系ＱＱ<a style="font-size: 11px;color: #9d9c9c;">（可更改会员中心头像）</a>
                            </div>
                ...
                <div class="text-center" style="padding: 15px 10px;">
                    <input type="submit" name="submit" class="btn submit_btn" style="width: 70%;padding:5px;" value="确定修改">
                </div>
            </div>
        </div>
    </div>
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 11px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/tixian.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>提现信息</span></a>
                    <a style="font-size: 14px; color: rgb(255, 91, 33); padding-left: 11px;"><span></span></a>
                    </div>
                    <div class="content-item-bottom">
                    <div style="padding: 10px 0;font-size: 1.3rem;color: #858585;">
                    ...
                    <div class="form-group form-group-transparent form-group-border-bottom">
                        <div class="input-group" style="width:100%">
                            <div class="input-group-addon" style="font-size: 10px;font-weight:500;color: #ff5938;">
                                提示：请如实填写正确的信息，填错乱填将打款失败
                            </div>
                        </div>
                    </div>
...
<?php
	...
	$jumoney=isset($_POST['jumoney'])?floatval($_POST['jumoney']):0;
	...
	if($sitename==null){
		...
		if($userrow['power']==2){
			...
			if($conf['fenzhan_edithtml']==1){
				$sql="UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`anounce`=:anounce,`modal`=:modal,`bottom`=:bottom,`alert`=:alert,`ktfz_price`=:ktfz_price,`ktfz_price2`=:ktfz_price2,`ktfz_domain`=:ktfz_domain,`template`=:template,`appurl`=:appurl,`jumoney`=:jumoney WHERE `zid`=:zid";
				$data = [':sitename'=>$sitename, ':title'=>$title, ':keywords'=>$keywords, ':description'=>$description, ':kfqq'=>$kfqq, ':kfwx'=>$kfwx, ':anounce'=>$anounce, ':modal'=>$modal, ':bottom'=>$bottom, ':alert'=>$alert, ':ktfz_price'=>$ktfz_price, ':ktfz_price2'=>$ktfz_price2, ':ktfz_domain'=>$ktfz_domain, ':template'=>$template, ':appurl'=>$appurl, ':jumoney'=>$jumoney, ':zid'=>$userrow['zid']];
			}else{
				$sql="UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`ktfz_price`=:ktfz_price,`ktfz_price2`=:ktfz_price2,`ktfz_domain`=:ktfz_domain,`template`=:template,`appurl`=:appurl,`jumoney`=:jumoney WHERE `zid`=:zid";
				$data = [':sitename'=>$sitename, ':title'=>$title, ':keywords'=>$keywords, ':description'=>$description, ':kfqq'=>$kfqq, ':kfwx'=>$kfwx, ':ktfz_price'=>$ktfz_price, ':ktfz_price2'=>$ktfz_price2, ':ktfz_domain'=>$ktfz_domain, ':template'=>$template, ':appurl'=>$appurl, ':jumoney'=>$jumoney, ':zid'=>$userrow['zid']];
			}
			$sds=$DB->exec($sql, $data);
		}else{
			if($conf['fenzhan_edithtml']==1){
				$sql="UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`anounce`=:anounce,`modal`=:modal,`bottom`=:bottom,`alert`=:alert,`template`=:template,`appurl`=:appurl,`jumoney`=:jumoney WHERE `zid`=:zid";
				$data = [':sitename'=>$sitename, ':title'=>$title, ':keywords'=>$keywords, ':description'=>$description, ':kfqq'=>$kfqq, ':kfwx'=>$kfwx, ':anounce'=>$anounce, ':modal'=>$modal, ':bottom'=>$bottom, ':alert'=>$alert, ':template'=>$template, ':appurl'=>$appurl, ':jumoney'=>$jumoney, ':zid'=>$userrow['zid']];
			}else{
				$sql="UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`template`=:template,`appurl`=:appurl,`jumoney`=:jumoney WHERE `zid`=:zid";
				$data = [':sitename'=>$sitename, ':title'=>$title, ':keywords'=>$keywords, ':description'=>$description, ':kfqq'=>$kfqq, ':kfwx'=>$kfwx, ':template'=>$template, ':appurl'=>$appurl, ':jumoney'=>$jumoney, ':zid'=>$userrow['zid']];
			}
			$sds=$DB->exec($sql, $data);
		}
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 80px;">
        <div class="content-item">
            <div class="content-item-top">
                    ...
                    <img src="../assets/img/icon/xinxi.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>网站信息</span></a>
                    <a style="font-size: 14px; color: rgb(255, 91, 33); padding-left: 11px;"><span></span></a>
                    </div>
                    <div class="content-item-bottom">
                    <div style="padding: 10px 0;font-size: 1.3rem;color: #858585;">
...
<?php  if($userrow['power']==1){?>
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 11px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/jiage.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>价格设定</span></a>
                    <a style="font-size: 11px; color: rgb(255, 91, 33); padding-left: 11px;"><span></span></a>
                    </div>
                    <div class="content-item-bottom">
                    <div style="padding: 10px 0;font-size: 1.3rem;color: #858585;">
                    <?php  if($userrow['uu']==1){?>
                        ...
                        <div class="input-group" style="width:100%">
                          <div class="input-group-addon">
                            短剧永久VIP价格<a style="color: #969494;font-size: 12px;">（提成50%）</a >
                          </div>
                        <input type="text" name="jumoney" placeholder="请输入价格" value="<?php echo ($userrow['jumoney']!= 0)? $userrow['jumoney'] : '';?>" id="inputValue" class="form-control">
            </div>
        </div>
        <div class="fui-cell-group fui-cell-click max-width" style="width: 95%;padding: 2px 5px 10px 5px;align-items: center;margin: 0px auto;border-radius: 15px">
            <div class="fui-icon-group selecter" >
                <a class="fui-icon-col external" style="color: #666666;font-size: 11px;">
                    短剧永久VIP价格介绍：不填留空将默认价格为2元，设置价格最低不能低于2元，最高不能超过99元。
                </a>
            </div>
        </div>
<script>
        ...
        input.value = '';
                    ...
                    <?php  if($userrow['uu']==0){?>
                         <div class="form-group form-group-transparent form-group-border-bottom">
                            <div class="input-group" style="width:100%">
                                <div class="input-group-addon">
                                    短剧永久VIP价格<a style="color: #969494;font-size: 12px;"></a>
                                </div>
                                <input type="text" style="font-size: 13px;" name="domain" value="未开通VIP会员,无法设置" class="form-control" disabled="">
                            </div>
                        </div>          
                    <?php }?>
                </div>
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 11px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/jiage.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>价格设定</span></a>
                    <a style="font-size: 11px; color: rgb(255, 91, 33); padding-left: 11px;"><span>下级开通,平台会收取部分成本费用</span></a>
                    </div>
                    <div class="content-item-bottom">
                    <div style="padding: 10px 0;font-size: 1.3rem;color: #858585;">
                        <div class="form-group form-group-transparent form-group-border-bottom">
                            <div class="input-group" style="width:100%">
                                ...
                                <input type="text" name="ktfz_price" placeholder="请输入价格" value="<?php echo $userrow['ktfz_price']>0?$userrow['ktfz_price']:$conf['fenzhan_price']; ?>" class="form-control">
                            </div>
                        </div>
        <div class="fui-cell-group fui-cell-click max-width" style="width: 95%;padding: 2px 5px 10px 5px;align-items: center;margin: 0px auto;border-radius: 15px">
            <div class="fui-icon-group selecter" >
                <a class="fui-icon-col external" style="color: #666666;font-size: 11px;">
                    分站站长价格介绍：最低不能低于0.01元。
                </a>
            </div>
        </div>
                        ...
                        <div class="form-group form-group-transparent form-group-border-bottom">
                            <div class="input-group" style="width:100%">
                                ...
                                <input type="text" name="ktfz_price2" placeholder="请输入价格" value="<?php echo $userrow['ktfz_price2']>$conf['fenzhan_cost2']?$userrow['ktfz_price2']:$conf['fenzhan_price2']; ?>" class="form-control">
                            </div>
                        </div>
        <div class="fui-cell-group fui-cell-click max-width" style="width: 95%;padding: 2px 5px 10px 5px;align-items: center;margin: 0px auto;border-radius: 15px">
            <div class="fui-icon-group selecter" >
                <a class="fui-icon-col external" style="color: #666666;font-size: 11px;">
                    顶级合伙人价格介绍：最低不能低于<?php echo $conf['fenzhan_cost2']+1?>元，最高不能超过<?php echo $conf['fenzhan_costgao']?>元。
                </a>
            </div>
        </div>
                    <?php  if($userrow['uu']==1){?>
...
<?php }?>
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; padding-top: 11px;">
        <div class="content-item">
            <div class="content-item-top">
                    <img src="../assets/img/icon/gongneng.png" style="width: 20px; margin-right: 10px;">
                    <a style="font-weight: 600; font-size: 16px; margin-right: 8px;"><span>功能设定</span></a>
                    <a style="font-size: 11px; color: rgb(255, 91, 33); padding-left: 11px;"><span></span></a>
                    </div>
          ...
          <select name="defaultcid" id="defaultcid" class="form-control" default="">
                <option value="<?php echo $userrow['defaultcid'] == 0 || empty($userrow['defaultcid'])? '' : $userrow['defaultcid'];?>" <?php echo $userrow['defaultcid'] == 0 || empty($userrow['defaultcid'])?'selected' : '';?>>
    <?php 
        if ($userrow['defaultcid'] == 0 || empty($userrow['defaultcid'])) {
            echo '未选择商品分类（可不选）';
        } else {
            echo '当前选择的分类ID为 '. $userrow['defaultcid'];
        }
   ?>
</option>
<option value="0" >恢复默认</option>
                        <?php
$rs=$DB->query("SELECT * FROM pre_class WHERE active=1  ORDER BY sort ASC");
while($res = $rs->fetch())
{
	echo '<option value="'.$res['cid'].'">'.$res['name'].'（ID:'.$res['cid'].'）</option>';
}
?>
                         </select>
        ...
        <div class="fui-cell-group fui-cell-click max-width" style="width: 95%;padding: 2px 5px 10px 5px;align-items: center;margin: 0px auto;border-radius: 15px">
                ...
                <div class="fui-icon-group selecter" >
                    <a class="fui-icon-col external" style="color: #666666;font-size: 11px;">
                        显示分类介绍：该功能主要用于首页商品可以自己选择显示某个分类下的，比如想首页全显示PDD助力的，就可以在此选项里选择PDD助力的分类，大分类就在一级分类里选，想只显示小分类比如只显示网赚项目分类的，就找到网创课程分类下的网赚项目即可，其它分类同理，若不选择则显示系统默认分类。
                    </a>
                </div>
            </div>
                    ...
                    <select name="fukaig" class="form-control">
                    <option value="0" <?php if ($userrow['fukaig'] == 0) echo "selected";?>>不显示</option>
                    <option value="1" <?php if ($userrow['fukaig'] == 1) echo "selected";?>>显示</option>
            </select>
    </div>
</div>
        <div class="fui-cell-group fui-cell-click max-width" style="width: 95%;padding: 2px 5px 10px 5px;align-items: center;margin: 10px auto;border-radius: 15px">
                <div class="fui-icon-group selecter" >
                    <a class="fui-icon-col external" style="color: #666666;font-size: 11px;">
                        显示群聊介绍：该功能主要用于首页付费群聊显示，若不想显示可设置不显示。
                    </a>
                </div>
            </div>
            <!--    </div>
                   <div style="padding: 10px 0;font-size: 1.3rem;color: #858585;">
                    ...
                    <select name="xian" class="form-control">
                    <option value="0" <?php if ($userrow['xian'] == 0) echo "selected";?>>不显示</option>
                    <option value="1" <?php if ($userrow['xian'] == 1) echo "selected";?>>显示</option>
            </select>
    </div>
</div>-->
                </div>
