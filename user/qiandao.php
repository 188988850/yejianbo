...
if ($row = $DB->getRow("SELECT * FROM shua_qiandao WHERE zid='{$userrow['zid']}' AND date='$day' ORDER BY id DESC LIMIT 1")) {
	$isqiandao = true;
...
$rs=$DB->query("SELECT * FROM shua_qiandao ORDER BY id DESC LIMIT 5");
$qqrow=array();
$qdrow=array();
while($res = $rs->fetch()){
	if(count($qqrow)<5){
...
<div class="col-xs-12 col-sm-10 col-md-6 col-lg-4 center-block " style="float: none; background-color:#fff;padding:0;max-width: 550px;">
    ...
    opacity: .8;align-items: center;justify-content: space-between;display: flex; flex-direction: row;height: 30px;">
            ...
            <font><a href="">每日签到</a></font>

            </div>
                </div>
        <div  style="height: 100%;text-align:center;background-color: #ffa56a;">
...
<div class="col">
 <div class="gg"><em>广告</em></div>
      <div class="text1"><font><a href="javascript:layer.alert('租此广告位联系微信：  ')" style="color: #999999;">此广告位招租中</a></font></div>
                </div>
        <div class="col">
            <div class="gg"><em>广告</em></div><!--qixiang/10.28-->
                <div class="text1"><font><a href="http://idc.521.al" target="_blank" style="color: #ff0000;">国内/海外低价高防/云服务器/Cdn</a></font></div>
                </div>
        <div class="col">
            <div class="gg"><em>广告</em></div><!--shunyi/10.25-->
                <div class="text1"><font><a href="http://pay.521.al" target="_blank" style="color: #0000ff;">菜鸟易支付/微信支付宝费率低/回款稳定</a></font></div>
                </div>
    </div>
</div>
<?php }?>
...
$(document).ready(function(){
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#count3').html( parseInt(data.count3) + 114666);
