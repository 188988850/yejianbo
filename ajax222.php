...
case 'group_query':
	...
	$re = $DB->query("SELECT A.*,B.`name` FROM `pre_group` A LEFT JOIN `shua_tools` B ON A.`tid`=B.`tid` WHERE A.`zid`='$zid' ORDER BY A.`id` DESC LIMIT 30");
	$ar_log = [];
	while ($res = $re->fetch()) {
		$ar_log[] = [
			'id' => $res['id'],
...
case 'payrmb':
	...
	exit(json_encode($result));
	    	break;
	    		case 'cidrs2':
	    		    	$cid=intval($_GET['cid']);
	    		    $rs=$DB->query("SELECT * FROM shua_class WHERE cid='$cid'");
	    		    $res = $rs->fetch();
	    		    $cy=strlen($res['nr']);
...
case 'gettool':
	...
	while($res = $rs->fetch(PDO::FETCH_ASSOC)){
		...
		$invitegift_money = 0;
		$invite_gift = 0;
		if($invite_row=$DB->getRow("SELECT * FROM shua_invitegiftshop WHERE tid='{$res['tid']}' LIMIT 1")){
			$isinvitegift = 1;
...
case 'gettoolnew':
    ...
    $subQuery = "(SELECT tid, IFNULL(SUM(IF(zid={$zid}, time, 0)), 0) AS time FROM shua_fenzhan_top GROUP BY tid)";
    ...
    $class_two = '';
    $class_two_values = explode('|', $conf['class_two']);
    if ($cid && in_array($cid, $class_two_values)) {
        $class_two = 'changeClassrol';
    }
	...
	if ($conf['template_store_sort_type'] == 1) {
        ...
        $orderBy = "top DESC,toptime DESC,addtime DESC,last ASC,tid DESC";
    }
	...
	$orderBy = "IFNULL(t.time, 0) DESC, " . $orderBy;
	...
	$where .= " and ((tg_type=1 and pro='{$zid}') or tg_type=0)";
	
	$sf = $DB->getRow("SELECT name FROM shua_class WHERE cid='$cid' limit 1")['name'];
	if(empty($cid) && !empty($conf['defaultcid'])){
	    //$_SESSION['vtid'] = $conf['defaultcid'];
	    $cid = $conf['defaultcid'];
	}
	$ic1=0;  $ic2=0;
	if($cid){
	  ...
	  if( $_SESSION['vtid']>1){
	   ...
	   //   $rs3=$DB->query("SELECT *,round(price,4) as price2 FROM shua_tools WHERE cid='$erjiid' AND tid<=$tt ORDER BY $orderBy LIMIT $page,$limit");
	  
	  $sql = "SELECT p.*,round(price,4) as price2,IFNULL(t.time, 0) AS time FROM shua_tools p LEFT JOIN ($subQuery) t ON p.tid = t.tid WHERE cid='$erjiid' AND tid<=$tt ORDER BY $orderBy LIMIT $page,$limit";
	  ...
	  $rs3=$DB->query($sql);
	    ...
	    $sql = "SELECT p.*,round(price,4) as price2,IFNULL(t.time, 0) AS time FROM shua_tools p LEFT JOIN ($subQuery) t ON p.tid = t.tid WHERE cid='$erjiid' ORDER BY $orderBy LIMIT $page,$limit";  
	...
	//tid AND tid<=1860 正序改成>   倒序代为<  
	 session_start();
	  if( $_SESSION['vtid']>1){
	      ...
	      $rs = $DB->query("SELECT p.*,IFNULL(t.time, 0) AS time FROM shua_tools p LEFT JOIN ($subQuery) t ON p.tid = t.tid WHERE $where AND tid<=$tt ORDER BY time DESC, {$orderBy} LIMIT $page, $limit");
	      
	      //$rs=$DB->query("SELECT * FROM shua_tools WHERE $where AND tid<=$tt ORDER BY $orderBy LIMIT $page,$limit");
	      
	  }else{
	      
	      
	      $rs = $DB->query("SELECT p.*,IFNULL(t.time, 0) AS time FROM shua_tools p LEFT JOIN ($subQuery) t ON p.tid = t.tid WHERE $where ORDER BY time DESC, {$orderBy} LIMIT $page, $limit");
	...
	while($res = $rs->fetch(PDO::FETCH_ASSOC)){
		...
		$data[]=array('tid'=>$res['tid'],'cid'=>$res['cid'],'sort'=>$res['sort'],'name'=>$res['name'],'value'=>$res['value'],'price'=>$price,'input'=>$res['input'],'inputs'=>$res['inputs'],'desc'=>$res['desc'],'alert'=>$res['alert'],'shopimg'=>$res['shopimg'],'repeat'=>$res['repeat'],'multi'=>$res['multi'],'close'=>$res['close'],'prices'=>$res['prices'],'min'=>$res['min'],'max'=>$res['max'],'sales'=>$res['sales'],'stock'=>$count,'isfaka'=>$isfaka,'addtime'=>strtotime($res['addtime']),'is_stock_err'=>$is_stock_err,'active'=>$res['active'],'class_two'=>$class_two);
	...
	$result=array("code"=>0,"msg"=>"succ",'sf'=>$sf,"data"=>$data,"info"=>$info,'pages'=>$pages,'total'=>intval($num2));
	exit(json_encode($result));
	break;
case 'getleftcount':
	$tid=intval($_POST['tid']);
...
// 			if(!$price)exit('{"code":-1,"msg":"当前商品批发价格优惠设置不正确"}');
		...
		if($need==0 && $tid!=$_SESSION['gift_tid']){
			...
			$thtime=date("Y-m-d").' 00:00:00';
			if($_SESSION['blockfree']==true || $DB->getColumn("SELECT count(*) FROM `shua_pay` WHERE `money`=0 AND `ip`='$clientip' AND `status`=1 AND `addtime`>'$thtime'")>=1){
