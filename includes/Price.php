...
class Price {
	...
	public function setvPriceInfo($vid,$del,$price,$cost=0){
		...
		if($price != $this->tool['price'] || $cost>0 && $cost != $this->tool['cost'] || $del != $this->vprice_array[$tid]['del']){
		...
		return $DB->exec("UPDATE pre_site SET vprice='$price_data' WHERE zid='{$this->zid}'");
	}
	
	public function setvbPriceInfo($vid,$del,$price,$cost=0){
		global $DB;
		$this->vbprice_array[$vid] = array();
		if($price != $this->tool['bfprice'] || $cost>0 && $cost != $this->tool['bfcost'] || $del != $this->vbprice_array[$tid]['del']){
	...
	private function getVideoInfo($vid){
		global $DB;
		$row=$DB->getRow("SELECT * FROM pre_videolist WHERE id='$vid' LIMIT 1");
		return $row;
	}
	...
	private function getdbVideoInfo($vid){
		global $DB;
		$row=$DB->getRow("SELECT * FROM pre_video WHERE id='$vid' LIMIT 1");
		return $row;
	}
