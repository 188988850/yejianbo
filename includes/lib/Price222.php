...
class Price {
	...
	public function setToolProfit($tid,$num,$name,$money,$orderid,$userid=0){
		...
		global $DB,$islogin2,$conf,$date;
			$ispay = $DB->getRow("SELECT tradeno FROM shua_orders WHERE id='$orderid'");
			$ddid=$ispay['tradeno'];
			$ispay2=$DB->getRow("SELECT type FROM shua_pay WHERE trade_no='$ddid'");
            if($ispay2['type']!='rmb'){
	           addPointRecord($userid,$money, '消费', '购买 '.$name.' (订单号:'.$orderid.')', $orderid);
	        }
	    
	   //查询商品数据     
	   $toolsinfo = $this->getToolInfo($tid);
	   
	   //如果是分站投稿商品不参与分销
	   if($toolsinfo['pro'] > 0 && $toolsinfo['tg_type'] == 1){
	       
	       $toolPrice = $this->getFinalPrice($this->getToolPrice($tid), $num);
	       
	       if(round($toolPrice*$num,2) != round($money,2))return false;
	       
	       $fenzhan_point = 0.9; //分站投稿提成百分比
	       $profit=$toolPrice;
	       $tc_point=round($profit*$num*$fenzhan_point, 2);
	       $rs=$this->changeUserMoney($this->zid, $tc_point, '购买投稿', '你网站有人下单'.$name.',获得'.$tc_point.'元提成', $orderid);
		...
		$toolPrice = $this->getFinalPrice($this->getToolPrice($tid), $num);
		if(round($toolPrice*$num,2) != round($money,2))return false;
		$fenzhan_money = 0.1;//低于这个价格不分销
	    $daili_point = 0.1;//代理提成百分比
	    $fenzhan_point = 0.9;//分站提成百分比
		if($this->power==2){
			$profit=$toolPrice - $this->getToolCost2($tid);
			if($profit>0 && $profit<$money){
				$tc_point=round($profit*$num, 2);
				$power_tc_point  = $tc_point * $daili_point;
				$rs=$this->changeUserMoney($this->zid, $tc_point * $fenzhan_point, '提成', '你网站有人下单'.$name.',获得'.$tc_point * $fenzhan_point.'元提成', $orderid);
			}
		}elseif($this->power==1){
			...
			if($profit2>0 && $profit2<$money && $this->upzid>1){
				...
				$power_tc_point  = $tc_point * $daili_point;
				$rs=$this->changeUserMoney($this->upzid, $tc_point * $fenzhan_point, '提成', '你下级分站站长有人下单'.$name.',获得'.$tc_point * $fenzhan_point.'元提成', $orderid);
			}
		}
		
		if(($tc_point * $fenzhan_point) >= $fenzhan_money){
