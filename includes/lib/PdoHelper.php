...
class PdoHelper
{
	...
	function __construct($dbconfig)
	{
		...
		try {
			// 优先用dbconfig中的charset，否则utf8mb4
			$charset = isset($dbconfig['charset']) ? $dbconfig['charset'] : 'utf8mb4';
			$this->db = new \PDO("mysql:host={$dbconfig['host']};dbname={$dbconfig['dbname']};port={$dbconfig['port']};charset=".$charset, $dbconfig['user'], $dbconfig['pwd']);
		} catch (\PDOException $e) {
			if (strpos($e->getMessage(), 'Unknown character set') !== false && $charset !== 'utf8') {
				try {
					$this->db = new \PDO("mysql:host={$dbconfig['host']};dbname={$dbconfig['dbname']};port={$dbconfig['port']};charset=utf8", $dbconfig['user'], $dbconfig['pwd']);
				} catch (\PDOException $e2) {
					exit('链接数据库失败:' . $e2->getMessage());
				}
			} else {
