...
class third_yunbao{
	...
	public function pre_check($tool, $num){
		...
		if($stock!==false && $stock!==null && $stock!==$tool['stock']){
			$DB->exec("update `shua_tools` set `stock`='$stock',`uptime`='".time()."' where `tid`='{$tool['tid']}'");
			if($num>$stock){
				return ['code' => -1, 'msg'=>'当前商品库存不足，无法购买'];
			}
		}else{
			$DB->exec("update `shua_tools` set `uptime`='".time()."' where `tid`='{$tool['tid']}'");
		}
		return ['code' => 0];
	}
	...
	public function pricejk($shequid, &$success)
	{
		...
		for($i=0;$i<10;$i++){
			$tool=$DB->getRow("SELECT * FROM shua_tools WHERE is_curl=2 AND shequ='{$shequid}' AND active=1 AND cid IN ({$conf['pricejk_cid']}) AND uptime<'".(time()-$pricejk_time)."' AND goods_type=1 ORDER BY uptime ASC");
			if(!$tool)break;
