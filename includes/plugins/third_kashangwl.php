...
class third_kashangwl{
	...
	public function pricejk_one($tool){
		...
		if(is_array($details)){
			$rs2=$DB->query("SELECT * FROM shua_tools WHERE is_curl=2 AND shequ={$tool['shequ']} AND goods_id={$tool['goods_id']}");
			while($res2 = $rs2->fetch())
			{
				...
				if($conf['pricejk_edit']==1 && $price>$res2['price'] && $res2['prid']>0){
					$DB->exec("update `shua_tools` set `price` ='{$price}' where `tid`='{$res2['tid']}'");
					$success++;
				...
				$DB->exec("update `shua_tools` set `uptime`='".time()."' where `tid`='{$res2['tid']}'");
			}
		}elseif(strpos($details,'商品不存在')!==false){
			$rs2=$DB->query("SELECT * FROM shua_tools WHERE is_curl=2 AND shequ={$tool['shequ']} AND goods_id={$tool['goods_id']}");
			while($res2 = $rs2->fetch())
			{
				$DB->exec("update `shua_tools` set `close`=1,`uptime`='".time()."' where `tid`='{$res2['tid']}'");
				$success++;
			}
		}
		return $success;
	...
	public function pricejk($shequid,&$success){
		...
		for($i=0;$i<30;$i++){
			$tool=$DB->getRow("SELECT * FROM shua_tools WHERE is_curl=2 AND shequ='{$shequid}' AND active=1 AND cid IN ({$conf['pricejk_cid']}) AND uptime<'".(time()-$pricejk_time)."' ORDER BY uptime ASC");
			if(!$tool)break;
	...
	public function notify($order){
		...
		if(isset($arr['order_id']) && $this->getSign($arr, $this->config['password']) == $arr['sign']){
			if($order['status']==2 && $arr['state']==200){
				$DB->exec("update shua_orders set status=1 where id='{$order['id']}'");
			}elseif($order['status']==2 && $arr['state']==500){
				$DB->exec("update shua_orders set status=3 where id='{$order['id']}'");
				if($arr['state_info']){
					$DB->exec("update shua_orders set result=:result where id=:id", [':result'=>$arr['state_info'], ':id'=>$order['id']]);
				}
			}
			echo 'ok';
		}else{
