...
if(isset($_COOKIE["admin_token"]))
{
	...
	if($admintypeid == '1'){
		if($adminuserrow = $DB->getRow("SELECT * FROM shua_account WHERE id='".intval($user)."' LIMIT 1")){
...
if(isset($_COOKIE["user_token"]))
{
	...
	if($userrow = $DB->getRow("SELECT * FROM shua_site WHERE zid='".intval($zid)."' LIMIT 1")){
		$session=md5($userrow['user'].$userrow['pwd'].$password_hash);
		if($session===$sid && $userrow['status']==1) {
			$islogin2=1;
			$_SESSION['zid'] = $userrow['zid'];
		}
	}
}
